...
try{for(const locale of ['zh','en']){
 ...
 const scrollHint=page.locator('.ng-uno-scroll-hint');
 ...
 for(const [width,height]of sizes){await page.setViewportSize({width,height});await begin('long-hand','&players=10');await expect(hand).toHaveCount(28);await expect(scrollHint).toBeVisible();await fits();await hand.last().scrollIntoViewIfNeeded();await fits();await page.screenshot({path:`${out}/${locale}-rack-${width}.png`});
 ...
 for(const [width,height]of sizes){
  await page.setViewportSize({width,height});await begin('long-hand','&players=10&waiting=1');
  await expect(scrollHint).toBeVisible();await expect(scrollHint).toContainText(locale==='zh'?'手牌可左右滑动':'Swipe to see your hand');await expect(play).toHaveCount(0);await fits();
  const hint=await scrollHint.boundingBox();assert(hint&&hint.x>=0&&hint.y>=0&&hint.x+hint.width<=width+1&&hint.y+hint.height<=height+1,'waiting hand hint remains on screen');
  await hand.last().scrollIntoViewIfNeeded();await expect(hand.last()).toBeInViewport();await fits();
  await page.screenshot({path:`${out}/${locale}-waiting-rack-${width}.png`});
 }
 await page.setViewportSize({width:1440,height:900});await begin();await expect(scrollHint).toHaveCount(0);
 await page.setViewportSize({width:320,height:568});await expect(scrollHint).toBeVisible();
 await page.setViewportSize({width:1440,height:900});await expect(scrollHint).toHaveCount(0);
 ...
 assert.deepEqual(errors,[]);console.log(`PASS ${locale}: single-row 28-card rack, visible overflow hint on waiting turns, hint clears when hand fits, 10 players, eight sizes, 44px controls, double-click/tap, wild color choice, keyboard, seat reset, rotation, draw and challenge`);await context.close();
