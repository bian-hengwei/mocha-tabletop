...
async function fullHand(locale,response,declared){
 ...
 await page.locator('.classic-controls .primary').click();
 await expect(page.locator('.latest-play .played-caption span').first()).toHaveText(comboName(locale,declared,'K'));
...
try{
 for(const locale of ['zh','en']){
  ...
  for(const declared of [false,true]){
   ...
   await expect(page.locator('.latest-play .played-caption span').first()).toHaveText(comboName(locale,declared,'9'));
