...
try{
 ...
 for(const locale of ['zh','en']){await page.getByRole('combobox').selectOption(locale);for(const [width,height]of [[320,568],[390,844],[430,932],[844,390],[932,430],[768,1024],[1440,900]]){await page.setViewportSize({width,height});await page.locator('.reaction-admin').evaluate(el=>el.scrollTo(0,0));await page.screenshot({animations:'disabled',path:`${out}/${locale}-${width}-top.png`});await page.getByRole('button',{name:locale==='zh'?'上架':'Publish',exact:true}).scrollIntoViewIfNeeded();await page.screenshot({animations:'disabled',path:`${out}/${locale}-${width}-list.png`});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));assert(await page.locator('.reaction-admin').evaluate(el=>el.scrollWidth<=el.clientWidth));}}
 ...
 for(const mode of ['cloud','lan']){
  ...
  await host.locator('.lobby-seats .social-avatar-button').click();
  for(const locale of ['zh','en']){if(locale==='en'){await host.keyboard.press('Escape');await host.locator('.language-toggle').click();await host.locator('.lobby-seats .social-avatar-button').click();}for(const [width,height] of [[320,568],[390,844],[430,932],[844,390],[932,430],[768,1024],[1440,900]]){await host.setViewportSize({width,height});await expect(host.getByRole('button',{name:locale==='zh'?'测试奶牛':'Test cow',exact:true})).toBeVisible();await host.screenshot({animations:'disabled',path:`${out}/${mode}-${locale}-${width}-picker.png`});}}
  ...
  await host.keyboard.press('Escape');await expect(host.locator('.lobby-seats .social-avatar-button')).toBeFocused();await host.locator('.brand .icon').click();host.once('dialog',d=>d.accept());await host.getByRole('button',{name:/^(离开牌桌|Leave table)$/}).click();await host.context().close();await guest.context().close();
 }
 ...
 for(const locale of ['zh','en']){await page.getByRole('combobox').selectOption(locale);for(const [width,height]of [[320,568],[390,844],[430,932],[844,390],[932,430],[768,1024],[1440,900]]){await page.setViewportSize({width,height});await page.locator('li').last().scrollIntoViewIfNeeded();await page.screenshot({animations:'disabled',path:`${out}/${locale}-${width}-dense.png`});assert(await page.locator('.reaction-admin').evaluate(el=>el.scrollWidth<=el.clientWidth));}}
