...
try{for(const language of ['zh','en']){
 ...
 for(const viewport of sizes){
  await page.setViewportSize(viewport);await visit('range');await expect(page.locator('.action-sheet footer button')).toBeDisabled();await page.locator('.choice').nth(1).click();await expect(page.locator('.action-sheet footer button')).toBeEnabled();
 ...
 for(const scenario of ['undercover','guess','penalty']){
  ...
  },{scenario,language});await page.reload();
  ...
  else await page.locator('.wg-word').filter({has:page.locator('strong',{hasText:/^黄金$/})}).click();
