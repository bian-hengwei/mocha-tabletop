...
try{for(const locale of ['zh','en'])for(const [width,height] of [[320,568],[390,844],[430,932],[844,390],[932,430],[768,1024],[1440,900],[568,320]]){
 ...
 await page.goto(`${base}/tests/ui/bombs-viewport.fixture.html?scenario=${scenario}`);await page.locator(scenario==='finished'?'.ng-final-results':'.bt-table').waitFor();if(scenario==='selected')await page.locator('.bt-hand .bt-card').filter({hasText:locale==='zh'?'攻击':'Attack'}).first().click();await fit(scenario);
