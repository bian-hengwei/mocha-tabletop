...
await own.locator('.ng-spice-card:not([disabled])').filter({has:page.locator('.ng-merchant-icon',{hasText:'✦'})}).first().click();assert.equal(await page.locator('.action-sheet').count(),0,'Gain merchant executes directly');assert.equal(await own.locator('.ng-spice-card').count(),1);
for(const viewport of [{width:320,height:568},{width:568,height:320}]){
