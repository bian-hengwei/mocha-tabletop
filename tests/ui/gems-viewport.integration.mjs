...
try{for(const locale of ['zh','en'])for(const [width,height]of [[320,568],[390,844],[430,932],[568,320],[844,390],[932,430],[768,1024],[1440,900]]){
 ...
 await page.locator('.g-bank-gems').evaluate(e=>e.scrollLeft=e.scrollWidth);const last=await page.locator('.g-bank-gem').last().boundingBox(),bank=await page.locator('.g-bank-gems').boundingBox();assert(last.x>=bank.x-1&&last.x+last.width<=bank.x+bank.width+1,'last token accessible horizontally');
 await page.locator('.g-bank-gems').evaluate(e=>e.scrollLeft=0);for(const i of [0,1,2])await page.locator('.g-bank-gem').nth(i).click();await expect(page.locator('.g-take .primary')).toBeEnabled();await page.locator('.g-take .primary').click();await expect(page.locator('.gem-picked')).toHaveCount(0);await fits(page);await page.screenshot({path:`${out}/${locale}-${width}.png`});
