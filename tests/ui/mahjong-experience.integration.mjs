...
try{for(const locale of ['zh','en']){
 ...
 await begin('long-names');await fit();assert(await page.locator('.mj-seat').evaluateAll(seats=>seats.every(seat=>{const outer=seat.getBoundingClientRect(),name=seat.querySelector('b').getBoundingClientRect();return name.left>=outer.left-1&&name.right<=outer.right+1;})),'long names stay within their seat');await page.screenshot({path:`${out}/${locale}-long-names.png`});
