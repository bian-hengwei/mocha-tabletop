...
try{
 for(const locale of ['zh','en']){
  ...
  const words=await page.locator('.wg-word>strong').allTextContents(),clue=['Journey','Harmony','Mystery'].find(word=>!words.includes(word));
  for(let turn=0;turn<3;turn++){
   ...
   await page.locator('.wg-key-toggle').click();await page.waitForFunction(()=>document.querySelector('.wg-key-toggle').getAttribute('aria-pressed')==='true');const target=await page.locator('.wg-word.wg-'+(captain===0?'red':'blue')+':not(.wg-revealed)').locator('.wg-cell-index').first().textContent();
   ...
   await seat(operative);await page.locator('.wg-word').nth(Number(target)-1).click();await confirm();await settled();await page.locator('.wg-guess-status .wg-button').click();await settled();captain=captain===0?1:0;operative=captain+2;
  }
