...
try{for(const locale of ['zh','en']){
 ...
 for(const [width,height] of sizes){await page.setViewportSize({width,height});
  ...
  assert.equal(await page.locator('.opponent-racks .classic-face:not(.card-back)').count(),0,'opponent racks never contain faces');
  assert.equal(await page.locator('.opponent-rack').count(),3);
  await expect(page.locator('.river .classic-face')).toHaveCount(96);
  await page.screenshot({path:`${out}/${locale}-mahjong-${width}.png`});
 }
 const winds=await page.locator('.mahjong-compass span').allTextContents();await page.getByRole('combobox',{name:'Seat',exact:true}).selectOption('1');
 const rotated=await page.locator('.mahjong-compass span').allTextContents();assert.notDeepEqual(rotated,winds,'wind labels follow the selected seat');
 await expect(page.locator('.seat-across b')).toContainText('Drew');await expect(page.locator('.river-self header b')).toHaveText(locale==='zh'?'南':'South');
 ...
 await seats.selectOption('practice-2');await expect(page.locator('.poker-play-area')).toHaveClass(/play-left/);await page.getByRole('button',{name:locale==='zh'?'不出':'Pass',exact:true}).click();await expect(page.locator('.seat-self .seat-call')).toHaveText(locale==='zh'?'不出':'Pass');
