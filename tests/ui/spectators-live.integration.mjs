...
try{for(const mode of (process.env.TEST_MODES?.split(',')||['cloud','lan'])){
 ...
 try{
  ...
  await join(guest);await host.getByRole('button',{name:'同意Player',exact:true}).click();await guest.getByRole('button',{name:'准备好了',exact:true}).waitFor();
  ...
  const late=await page('Late observer');await join(late);await host.locator('.audience-trigger').click();await host.getByRole('button',{name:'同意Late observer',exact:true}).click();await late.locator('.spectator-surface .g-table').waitFor();
