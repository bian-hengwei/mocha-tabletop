...
try{for(const mode of (process.env.TEST_MODES?.split(',')||['cloud','lan'])){
 ...
 async function page(name){const c=await browser.newContext({viewport:{width:390,height:844}});contexts.push(c);await c.addInitScript(name=>{localStorage.setItem('mocha-profile',JSON.stringify({id:crypto.randomUUID(),name,avatar:'🦊'}));localStorage.setItem('mocha-locale','zh');},name);const p=await c.newPage();p.setDefaultTimeout(15000);p.on('pageerror',e=>errors.push(e.message));await p.goto(base);return p;}
 try{
  ...
  for(const language of ['zh','en']){
   ...
   for(const [width,height]of [[320,568],[390,844],[430,932],[844,390],[932,430],[768,1024],[1440,900]]){await host.setViewportSize({width,height});await expect(host.getByRole('checkbox',{name:language==='zh'?'允许观战':'Allow spectators'})).toBeVisible();assert(await host.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'audience dialog fits');if(width===320||width===844)await host.screenshot({path:`${out}/${mode}-${language}-${width}.png`});}
  }
  host.once('dialog',d=>d.accept());await host.getByRole('checkbox',{name:'Allow spectators'}).click();await expect(host.getByRole('checkbox',{name:'Allow spectators'})).not.toBeChecked();await expect(watcher.locator('.spectator-surface')).toHaveCount(0);await expect(late.locator('.spectator-surface')).toHaveCount(0);await expect(host.locator('.spectator-list')).toHaveCount(0);
  await host.getByRole('checkbox',{name:'Allow spectators'}).click();await expect(host.getByRole('checkbox',{name:'Allow spectators'})).toBeChecked();await host.keyboard.press('Escape');await host.locator('.language-toggle').click();await join(watcher,false,true);await watcher.locator('.spectator-surface').waitFor();
