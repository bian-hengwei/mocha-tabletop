...
async function fits(page){
 ...
 const result=await page.evaluate(()=>{const rack=document.querySelector(':is(.classic-hand-scroll,.mj-hand-scroll)').getBoundingClientRect();const controls=[...document.querySelectorAll(':is(.classic-controls,.mj-actions) button')].map(e=>({text:e.textContent,...Object.fromEntries(['x','y','width','height','right','bottom'].map(k=>[k,e.getBoundingClientRect()[k]]))}));return{width:innerWidth,height:innerHeight,overflow:document.documentElement.scrollWidth>innerWidth,rack:{x:rack.x,y:rack.y,right:rack.right,bottom:rack.bottom},controls};});
 ...
 assert.equal(await page.locator(':is(.classic-hand,.mj-hand) .classic-card-art').count(),await page.locator(':is(.classic-hand,.mj-hand) :is(.classic-card,.mj-tile)').count(),'every card uses complete SVG artwork');
...
try{for(const locale of ['zh','en'])for(const [kind,mode]of [['doudizhu'],['guandan'],['mahjong','guangdong'],['mahjong','sichuan'],['mahjong','bloodflow'],['mahjong','laizi']].filter(([kind])=>!process.env.TEST_GAMES||process.env.TEST_GAMES.split(',').includes(kind))){
 ...
 for(const viewport of viewports){await page.setViewportSize(viewport);await fits(page);const last=page.locator(':is(.classic-hand,.mj-hand) :is(.classic-card,.mj-tile):not(:disabled)').last();await last.scrollIntoViewIfNeeded();await last.click();await expect(last).toHaveAttribute('aria-pressed','true');await fits(page);await page.screenshot({path:`${out}/${locale}-${kind}-${mode||'classic'}-${viewport.width}.png`});if(kind==='mahjong')await page.keyboard.press('Escape');else await last.click();}
 const last=page.locator(':is(.classic-hand,.mj-hand) :is(.classic-card,.mj-tile):not(:disabled)').last();await last.click();const name=await last.getAttribute('aria-label');await page.setViewportSize(viewports[1]);await expect(page.getByRole('button',{name,exact:true}).last()).toHaveAttribute('aria-pressed','true');await page.setViewportSize(viewports[3]);await expect(page.getByRole('button',{name,exact:true}).last()).toHaveAttribute('aria-pressed','true');await fits(page);
 if(kind==='guandan'){
  await page.getByRole('button',{name:locale==='zh'?'清空选择':'Clear selection',exact:true}).click();
  await page.locator(':is(.classic-hand,.mj-hand) :is(.classic-card,.mj-tile)').first().click();await page.locator('.classic-controls .primary').click();
