...
const url=process.env.BASE_URL||'http://127.0.0.1:5174',folder='test-results/bombs';await fs.mkdir(folder,{recursive:true});
...
for(const seed of process.env.LAYOUT_ONLY?[]:[9,20,43]){
 ...
 for(let step=0;step<350;step++){
  ...
  if(phase==='response'){
    ...
    for(const title of ['预知三张','索取','洗牌','攻击','跳过']){if((!covered.has(title)||(title==='预知三张'&&!covered.has('future')))&&titles.includes(title)){await hand.nth(titles.indexOf(title)).click();const play=page.getByRole('button',{name:'打出这张',exact:true});if(await play.count()){await play.click();covered.add(title);acted=true;break;}}}
