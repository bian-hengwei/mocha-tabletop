...
const base=process.env.TEST_API_BASE||'http://127.0.0.1:8787';if(!['127.0.0.1','localhost'].includes(new URL(base).hostname))throw Error('Discovery IP spoofing tests require a local emulator');const a='2001:db8:1234:5678::11',b='2001:db8:1234:5678::22',other='2001:db8:1234:5679::22';
