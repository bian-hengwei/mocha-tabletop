...
describe('LAN initial pairing recovery',()=>{
 it('retries a stalled new ICE state without failure events, with a finite signaling budget',async()=>{
  vi.useFakeTimers();vi.stubGlobal('window',new EventTarget());vi.stubGlobal('document',{addEventListener(){},removeEventListener(){}});
  ...
  const client=new RoomClient() as any;client.session={profile:{id:'host0000'}};client.stopped=false;client.ws={readyState:1,send(data:string){const message=JSON.parse(data);if(message.type==='signal')messages.push(message);},close(){}};
  ...
  await vi.advanceTimersByTimeAsync(120000);expect(connections).toHaveLength(5);expect(messages.every(message=>message.type==='signal')).toBe(true);expect(messages).toHaveLength(5);expect(client.state.room.mode).toBe('lan');expect(client.state.error).toContain('切换云端');window.dispatchEvent(new Event('pageshow'));await vi.advanceTimersByTimeAsync(1500);expect(connections).toHaveLength(6);client.destroy();expect(vi.getTimerCount()).toBe(0);
 });
});
