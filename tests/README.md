...
各组使用独立 runner、Vite 和本地 Worker，组内顺序执行。基础检查与集成组同时运行；集成脚本通过 Vite 读取源码，不依赖基础检查生成的 `dist/`。每个脚本输出名称、结果、耗时，并写入 Actions job summary。一组失败后该组剩余脚本标记为未运行，其他组继续完成以提供独立诊断。服务启动失败会及时终止并输出服务日志。
