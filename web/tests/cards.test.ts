...
import nativeGems from '../../Sources/MochaCore/GemsEngine.swift?raw';
...
describe('宝石商人 catalog/setup',()=>{
  it('ports all 90 native numerical components exactly',()=>{const swift=nativeGems;const native=swift.split('let raw = """')[1].split('"""')[0].trim().split('\n').map((l:string)=>l.trim());expect(GEM_CATALOG.map(c=>[c.id,c.tier,GEM_COLORS.indexOf(c.bonus),c.points,...c.cost].join(','))).toEqual(native);expect([1,2,3].map(t=>GEM_CATALOG.filter(c=>c.tier===t).length)).toEqual([40,30,20]);expect(NOBLE_CATALOG).toHaveLength(10);});
