...
const browser=await(safari?webkit.launch({headless:true}):chromium.launch({headless:true,executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'}));
...
try{
 ...
 for(const {title,count,selector,key,mode}of scenarios){
  ...
  await host.screenshot({path:`docs/previews/${safari?'webkit':'chrome'}-${key}-cloud.png`});
...
}catch(error){for(let i=0;i<pages.length;i++)await pages[i].screenshot({path:`/private/tmp/tt-acceptance-failure-${safari?'webkit':'chrome'}-${i}.png`}).catch(()=>{});throw error;}
