...
export function RoleArt({role,className='',align='center'}:{role:string;className?:string;align?:'top'|'center'}){
  const aliases:Record<string,string>={servant:'loyal',minion:'evil',moderator:'seer'};
  const index=Math.max(0,roleKeys.indexOf(aliases[role]||role));const tile=index%6;
...
export function CardArt({card,className=''}:{card:{id:string;tier:number};className?:string}){
  const tile=[...card.id].reduce((n,c)=>n*31+c.charCodeAt(0),0)>>>0;const v=tile%6;
...
export function GemArt({color,className=''}:{color:number;className?:string}){
