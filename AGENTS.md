...
- Run `git diff --check` and `npm run check`. The latter includes unit tests, Worker type checks, frontend type checks, and a production build.
- Select additional tests by change scope: independent rule scenarios and terminal states; both languages and dynamic branches; viewport and interaction checks; network authorization/recovery; persistence; or PWA update/offline behavior.
