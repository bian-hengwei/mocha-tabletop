...
斗地主和掼蛋的公开出牌按座位保留，后续玩家不会覆盖其他人的牌；该玩家再次出牌或不出时更新自己的记录。全员不出后仍显示上一轮，新一轮领出时统一清理，重新发牌时重置。记录保存在权威状态中，重连后恢复；旧存档只能恢复原本保存的最后一手。新出牌通过独立的公开牌面动画层，从出牌方移到中央放大展示，再落到座位旁；落牌后恢复静态牌组。动画约 0.9 秒，不阻塞下一位操作；刷新、换座不重播旧牌，旋转或开启减少动态效果会立即结束飞牌并显示牌组。叫分和等待回合只禁用手牌操作，不降低牌面透明度或饱和度。
