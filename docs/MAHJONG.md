...
General wins use four sets and a pair, or seven pairs; a natural quad can count as two pairs. Guangdong chicken-and-flat excludes seven pairs. Guangdong Push Down and both Guangdong scoring tables support thirteen orphans; the single-wild Guangdong scoring option permits wild substitution in it. Red Dragon Wild uses four sets and a pair or seven pairs. A missing suit must be cleared before winning or claiming. After passing a win, Sichuan tables block an equal/lower base-value discard win until the next draw; Guangdong scoring blocks the same passed tile value until drawing or claiming a meld.
