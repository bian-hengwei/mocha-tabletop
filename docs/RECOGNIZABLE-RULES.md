...
- 远征显示为「迷雾远征」，身份展示为先知、守望者、伪先知、刺客、远征队员与暗影同伴。规则和身份页解释各自知道什么、能做什么。英文采用 Seer / Watcher / False Seer / Assassin。
...
否决与漏喊使用全员响应窗口，避免实时计时延迟决定是否错失响应。猫牌中，打出功能牌、组合或否决本身计为本人的确认；其余存活玩家选择否决或继续。每张否决切换效果并重新开放其余玩家的响应，窗口结束前仍可反制或从手牌选择否决自己的出牌，不按手中有无否决牌跳过玩家。任务票和选牌保持同时揭晓。身份与牌堆仍由引擎过滤后交给各玩家。更换名字没有改变指令权限、状态结构、费用或胜负算法。
