...
import {mahjongModeNotes} from '../i18n/mahjongVariants';
...
export function RuleOptions({kind,options,onChange,disabled=false,playerCount}:{kind:GameKind;options:GameOptions;disabled?:boolean;playerCount?:number;onChange:(next:GameOptions)=>void}) {
 const winDescription=useId();
 const locale=useLocale();
 if(kind==='doudizhu'||kind==='guandan')return <div className="rule-options"><label className="uno-challenge-toggle"><span>{t('记牌器')}</span><input type="checkbox" role="switch" disabled={disabled} aria-label={t('记牌器')} checked={options.pokerCounter===true} onChange={e=>onChange({...options,pokerCounter:e.target.checked})}/><small>{t('显示除自己手牌外尚未打出的牌数')}</small></label></div>;
 if(kind==='mahjong')return <div className="rule-options"><label>{t('麻将玩法')}<select disabled={disabled} aria-label={t('麻将玩法')} value={options.mahjongMode||'guangdong'} onChange={e=>onChange({...options,mahjongMode:e.target.value as GameOptions['mahjongMode']})}>{Object.entries(MAHJONG_MODES).map(([key,label])=><option key={key} value={key}>{t(label)}</option>)}</select></label><small>{mahjongModeNotes[options.mahjongMode||'guangdong'][locale==='zh'?0:1]}</small></div>;
 ...
 return <div className="rule-options wolf-options"><label>{t('身份牌型')}<select disabled={disabled} aria-label={t('身份牌型')} value={preset.id} onChange={e=>onChange({...options,werewolfPreset:e.target.value as GameOptions['werewolfPreset']})}>{WEREWOLF_PRESETS.map(p=><option key={p.id} value={p.id}>{t(p.name)}</option>)}</select></label><label>{t('胜负条件')}<select disabled={disabled} aria-label={t('胜负条件')} aria-describedby={winDescription} value={options.werewolfWin||'sides'} onChange={e=>onChange({...options,werewolfWin:e.target.value as GameOptions['werewolfWin']})}><option value="sides">{t('屠边')}</option><option value="parity">{t('人数平衡')}</option></select></label>
 <p id={winDescription} className="wolf-win-description">{t(options.werewolfWin==='parity'?'狼人存活人数达到或超过好人时获胜。':'狼人清除全部平民或全部神职中的任一类时获胜。')}</p>
