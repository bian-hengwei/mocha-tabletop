...
function SushiTable(props: Props) { const { view, command, selfID } = props, b = view.board, [selected, setSelected] = useState<string[]>([]), pick = view.actions.find(a => a.id === 'pick'); useEffect(() => setSelected([]), [b.round, b.step, selfID, !!b.selected]); const taps=useCardDoubleTap([selfID,b.round,b.step,!!b.selected].join(':')); const shown = b.selected || selected; const toggle = (id: string) => setSelected(old => old.includes(id) ? old.filter(v => v !== id) : old.length < (pick?.max || 1) ? [...old, id] : [id]); const valid = pick && selected.length >= pick.min && selected.length <= pick.max; return <div className="ng-table ng-sushi"><div className="ng-sushi-overview"><Header view={view} selfID={selfID}/><Scoreboard view={view} selfID={selfID}/>{b.players.some((p: any)=>p.table.length>0)&&<section className="ng-plates">{tx(b.players.map((p: any) => <article className="ng-panel" key={p.id}><h3>{tx(p.avatar)} {p.name}<small>{t("盘面") + " "}{tx(sushiPlateScore(p.table))}{" " + t("分 · 卷数另算")}</small></h3><div className="ng-plate">{tx(p.table.length ? p.table.map((c: SushiCard) => <SushiFace key={c.id} card={c} small/>) : <p className="ng-hint">{t("等待大家同时揭晓第一道")}</p>)}</div></article>))}</section>}</div>{!view.spectating&&<><section className="ng-panel ng-sushi-hand-panel"><h3>{t("你的手牌") + " "}<small>{tx(b.hand.length)}{" " + t("张 · 仅你可见")}</small></h3><div className="ng-hand" tabIndex={0} aria-label={t("手牌可左右滑动")}>{tx((b.hand as SushiCard[]).map(c => <SushiFace key={c.id} card={c} selected={shown.includes(c.id)} order={shown.includes(c.id) ? shown.indexOf(c.id) + 1 : undefined} onPointerDown={taps.onPointerDown} onClick={pick ? event => {if(taps.isDoubleTap(c.id,event)&&pick.max===1){command({action:'pick',values:[c.id]});setSelected([]);}else toggle(c.id);} : undefined}/>))}</div>{view.actions.length>0&&<div className="ng-controls ng-sushi-confirm">{tx(pick && <button className="ng-action" type="button" disabled={!valid} onClick={() => { command({ action: 'pick', values: selected }); setSelected([]); }}>{selected.length?`${t("确认")} ${selected.length} ${t("张")}`:t("确认选牌")}{pick.max === 2 ? ' · 🥢 ×2' : ''}</button>)}<Controls {...props} omit={['pick']}/></div>}</section></>}</div>; }
...
function UnoTable(props: Props) {
 ...
 return <div className={`ng-table ng-uno ${challenge?'ng-reviewing':''}`}><div className="ng-uno-overview"><Header view={view} selfID={selfID}/><Scoreboard view={view} selfID={selfID}/><div className="ng-edition"><span>{t(`第 ${b.roundNumber} 轮`)}</span><span>{b.mode==='single'?t('单局竞速'):t('累计 500 分获胜')}</span><span>{t(b.challengeEnabled===false?'+4 质疑已关闭':'+4 质疑已开启')}</span></div>
  ...
  {!view.spectating&&<><section className="ng-panel ng-uno-hand-panel"><h3>{t('你的手牌')}<small>{b.hand.length} {t('张')}</small></h3><div className="ng-hand ng-uno-hand" tabIndex={0} aria-label={t('手牌可左右滑动')}>{(b.hand as UnoCard[]).map(c=><UnoFace card={c} key={c.id} drawn={b.drawn===c.id} selected={selectedID===c.id&&!!selectedAction} onPointerDown={taps.onPointerDown} onClick={actionFor(c)?e=>selectCard(c,e):undefined}/>)}</div>
