...
function SpiceLine({ text }: { text: string }) {
 return <>{t(text).split(/(🟡|🔴|🟢|🟤)/u).map((part,index)=>{const color=SPICE_SYMBOLS.indexOf(part);return color<0?<span key={index}>{part}</span>:<SpiceCube key={index} color={color}/>;})}</>;
}
