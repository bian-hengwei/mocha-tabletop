...
function CenturyTable(props: Props) {
    ...
    return <div className="ng-table ng-century"><div className="ng-century-overview"><Header view={view} selfID={selfID}/><Scoreboard view={view} selfID={selfID}/><p className="ng-edition">{t("香料商旅 · 基础牌组") + " "}<span>{t("完成") + " "}{tx(b.goalTarget)}{" " + t("单触发最后一轮")}</span></p><section className="ng-panel"><h3>{t("公开订单") + " "}<small>{t("金币余") + " "}{tx(b.gold)}{" " + t("· 银币余") + " "}{tx(b.silver)}</small></h3><div className="ng-market" tabIndex={0} aria-label={t("公开订单")}>{tx((b.goals as SpiceOrder[]).map((o, i) => { const onClick = choice('claim', o.id); return <button type="button" className="ng-spice-card ng-goal" key={o.id} disabled={!onClick} onClick={onClick}><span className="ng-spice-illustration"><IllustratedTile kind="century" index={i%2?5:4}/></span><small className="ng-order-bonus">{tx(i === 0 && b.gold ? '🟡 金币 +3' : ((i === 1 && b.gold) || (i === 0 && !b.gold)) && b.silver ? '⚪ 银币 +1' : '订单')}</small><strong>{tx(o.points)}<small>{" " + t("分")}</small></strong><span className="ng-order-cost"><SpiceLine text={spiceText(o.cost)}/></span>{tx(onClick && <b>{t("完成订单 →")}</b>)}</button>; }))}</div></section><section className="ng-panel"><h3>{t("商人市场") + " "}<small>{t("从左向右支付路费")}</small></h3><div className="ng-market" tabIndex={0} aria-label={t("商人市场")}>{tx(b.market.map((slot: {
            card: SpiceCard;
            bonus: number[];
        }, i: number) => { const onClick = choice('acquire', slot.card.id); return <button type="button" className="ng-spice-card ng-merchant-card" key={slot.card.id} disabled={!onClick} onClick={onClick}><small className="ng-route-price">{tx(i === 0 ? '免费招募' : `支付 ${i} 枚任选香料`)}</small><span className="ng-spice-illustration"><IllustratedTile kind="century" index={merchantIllustration(slot.card)}/></span><b><SpiceLine text={spiceCardTitle(slot.card)}/></b>{slot.bonus.some(Boolean)&&<small className="ng-merchant-bonus">{t("附赠：")}<SpiceLine text={spiceText(slot.bonus)}/></small>}</button>; }))}</div></section><section className="ng-plates">{tx(b.players.filter((p: any) => p.id !== selfID).map((p: any) => <article className="ng-panel" key={p.id}><h3>{tx(p.avatar)} {p.name}{" " + t("的商队")}</h3><Cubes values={p.cubes}/></article>))}</section><details className="ng-rules"><summary>{t("本桌规则与牌组说明")}</summary><p>{t("每回合使用一张商人、招募商人、完成一单或休整。升级可逐次选择同一枚或不同枚香料，也可提前停止；交易可以重复任意次，只要付得起。回合结束归还超过 10 枚的香料。招募时逐一支付给左侧商人，再获得该牌上的香料。")}</p><p>{t("两三人完成 6 单、四五人完成 5 单后完成当前整轮。订单、金币 3 分、银币 1 分和每枚非黄色香料 1 分相加；同分时座次靠后者胜。")}</p><p>{t("使用基础牌组：43 张市场商人、36 张订单，每人另有两张起始牌。商人效果、交换比例与订单分数均按原版数值逐张核对。")}</p></details><details className="ng-rules"><summary>{t("已用商人") + " "}{tx(b.played.length)}{" " + t("张 · 休整后收回")}</summary>{tx((b.played as SpiceCard[]).map(c => <p key={c.id}>{tx(spiceCardTitle(c))}</p>))}</details></div><div className="ng-century-hand-dock"><aside className={`ng-pocket ${pending?'ng-pocket-active':''}`} aria-label={t("你的商队")}><span>{t("你的商队")}<small>{b.cubes.reduce((sum: number, value: number) => sum + value, 0)} / 10</small></span><Cubes values={b.cubes}/>{pending&&<div className="ng-pending-actions">{b.active&&<p><SpiceLine text={spiceCardTitle(b.active)}/>{b.phase==='upgrade'&&t(` · 还可升级 ${b.remaining} 次`)}</p>}<Controls {...props} omit={['play','acquire','claim']}/></div>}</aside>{!pending&&<section className="ng-panel ng-century-hand-panel"><h3>{t("你的商队") + " "}<small>{tx(b.cubes.reduce((n: number, v: number) => n + v, 0))}{" " + t("/ 10 枚")}</small></h3><Cubes values={b.cubes}/>{!pending&&<Controls {...props} omit={['play', 'acquire', 'claim']}/>}<h3>{t("可用商人") + " "}<small>{tx(b.hand.length)}{" " + t("张")}</small></h3><div className="ng-market" tabIndex={0} aria-label={t("可用商人")}>{tx((b.hand as SpiceCard[]).map(c => { const onClick = choice('play', c.id); return <button type="button" aria-label={t(spiceCardTitle(c))} className="ng-spice-card ng-merchant-card" key={c.id} disabled={!onClick} onClick={onClick}><span className="ng-spice-illustration"><IllustratedTile kind="century" index={merchantIllustration(c)}/></span><b><SpiceLine text={spiceCardTitle(c)}/></b></button>; }))}</div></section>}</div></div>;
}
