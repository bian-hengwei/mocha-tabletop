...
import {CenturyOrderDialog} from './CenturyOrderDialog';
type Props = {
    view: GameView;
    selfID: string;
...
function Header({ view, selfID }: { view: GameView; selfID: string }) {
 ...
 const waiting=view.kind==='sushi'&&!view.finished&&board.selected?board.players.filter((player:{ready:boolean;name:string})=>!player.ready):[];
 return ordinaryTurn?null:<header className="ng-heading"><strong role="status">{waiting.length===1?<>{t('仍在选牌')}{t('：')}{waiting[0].name}</>:t(view.instruction)}</strong></header>;
}
...
function UnoTable(props: Props) {
 ...
 const handRef=useRef<HTMLDivElement>(null),[handOverflow,setHandOverflow]=useState(false);
 useLayoutEffect(()=>{
  const hand=handRef.current;if(!hand)return;
  const measure=()=>setHandOverflow(hand.scrollWidth>hand.clientWidth+2);
  const observer=new ResizeObserver(measure);observer.observe(hand);measure();
 ...
 return <div className={`ng-table ng-uno ${challenge?'ng-reviewing':''}`}><div className="ng-uno-overview"><Header view={view} selfID={selfID}/><Scoreboard view={view} selfID={selfID}/><div className="ng-edition"><span>{t(`第 ${b.roundNumber} 轮`)}</span><span>{b.mode==='single'?t('单局竞速'):t('累计 500 分获胜')}</span><span>{t(b.challengeEnabled===false?'+4 质疑已关闭':'+4 质疑已开启')}</span></div>
  ...
  <section className="ng-panel ng-uno-hand-panel"><h3>{t('你的手牌')}<small>{t(`${b.hand.length} 张`)}{handOverflow&&<span className="ng-hand-scroll-cue">↔ {t('滑动查看手牌')}</span>}</small>{!view.finished&&b.current===selfID&&b.phase==='play'&&<span className="ng-uno-turn-label" role="status">{t('轮到你了')}</span>}</h3><div ref={handRef} className="ng-hand ng-uno-hand" tabIndex={0} aria-label={t('手牌可左右滑动')}>{(b.hand as UnoCard[]).map(c=><UnoFace card={c} key={c.id} drawn={b.drawn===c.id} selected={selectedID===c.id&&!!selectedAction} onPointerDown={taps.onPointerDown} onClick={actionFor(c)?e=>selectCard(c,e):undefined}/>)}</div>
...
function CenturyTable(props: Props) {
    ...
    const order=(b.goals as SpiceOrder[]).find(o=>o.id===orderID);
    const claim=view.actions.find(a=>a.id==='claim'),canClaim=!!order&&!!claim?.choices.some(c=>c.id===order.id);
    const orderBonus=(index:number)=>index===0&&b.gold?'🟡 金币 +3':((index===1&&b.gold)||(index===0&&!b.gold))&&b.silver?'⚪ 银币 +1':'';
    useEffect(()=>setOrderID(null),[selfID,b.current,b.phase,b.round,view.finished]);
    const pending=!view.finished&&b.current===selfID&&b.phase!=='action';
    const handRef=useRef<HTMLDivElement>(null),[handOverflow,setHandOverflow]=useState(false);
    const handIDs=(b.hand as SpiceCard[]).map(card=>card.id).join('|');
    useLayoutEffect(()=>{
     const hand=handRef.current;if(!hand){setHandOverflow(false);return;}
     const measure=()=>setHandOverflow(hand.scrollWidth>hand.clientWidth+2);
     const observer=new ResizeObserver(measure);observer.observe(hand);
     for(const card of hand.children)observer.observe(card);
    ...
    return <div className="ng-table ng-century"><div className="ng-century-overview">{view.finished?<div className="ng-century-result-heading"><Header view={view} selfID={selfID}/>{props.onShowResults&&<button type="button" className="ng-action" onClick={props.onShowResults}>{t("得分明细")}</button>}</div>:<Header view={view} selfID={selfID}/>}<Scoreboard view={view} selfID={selfID}/><nav className="ng-century-tabs" aria-label={t("桌面区域")}>{[["market","商人市场"],["orders","公开订单"],["caravans","商队"],["used","已用商人"]].map(([id,label])=><button type="button" key={id} aria-pressed={section===id} onClick={()=>setSection(id)}>{t(label)}</button>)}</nav><section className="ng-panel ng-century-page" hidden={section!=="orders"}><h3>{t("公开订单")}<small>{t("目标")} {b.goalTarget}</small><small aria-label={`${t("金币余")} ${b.gold} ${t("· 银币余")} ${b.silver}`}>🟡 {b.gold} · ⚪ {b.silver}</small></h3><div className="ng-market" tabIndex={0} aria-label={t("公开订单")}>{tx((b.goals as SpiceOrder[]).map((o, i) => { const onClick = choice('claim', o.id); return <button type="button" className={`ng-spice-card ng-goal ${onClick?"ng-claimable":""}`} key={o.id} onClick={()=>setOrderID(o.id)} aria-label={`${t("订单详情")} · ${o.points} ${t("分")} · ${t(orderBonus(i)||"订单")} · ${t(spiceText(o.cost))}`}><span className="ng-spice-illustration"><IllustratedTile kind="century" index={i%2?5:4}/></span><small className="ng-order-bonus">{t(orderBonus(i)||'订单')}</small><strong>{tx(o.points)}<small>{" " + t("分")}</small></strong><span className="ng-order-cost"><SpiceLine text={spiceText(o.cost)}/></span><b>{t(onClick?"完成订单 →":"查看费用")}</b></button>; }))}</div></section><section className="ng-panel ng-century-page" hidden={section!=="market"}><h3>{t("商人市场")}</h3><div className="ng-market" tabIndex={0} aria-label={t("商人市场")}>{tx(b.market.map((slot: {
            card: SpiceCard;
            bonus: number[];
        }, i: number) => { const onClick = choice('acquire', slot.card.id); return <button type="button" className="ng-spice-card ng-merchant-card" key={slot.card.id} disabled={!onClick} onClick={onClick}><small className="ng-route-price">{tx(i === 0 ? '免费招募' : `支付 ${i} 枚任选香料`)}</small><span className="ng-spice-illustration"><IllustratedTile kind="century" index={merchantIllustration(slot.card)}/></span><b><SpiceLine text={spiceCardTitle(slot.card)}/></b>{slot.bonus.some(Boolean)&&<small className="ng-merchant-bonus">{t("附赠：")}<SpiceLine text={spiceText(slot.bonus)}/></small>}</button>; }))}</div></section><section className="ng-plates ng-century-page" hidden={section!=="caravans"}>{tx(b.players.filter((p: any) => p.id !== selfID).map((p: any) => <article className="ng-panel" key={p.id}><h3>{tx(p.avatar)} {p.name}{" " + t("的商队")}</h3><Cubes values={p.cubes}/></article>))}</section><section className="ng-panel ng-century-page" hidden={section!=="used"}><h3>{t("已用商人")}<small>{t(`${b.played.length} 张`)}</small></h3><div className="ng-market" tabIndex={0} aria-label={t("已用商人")}>{(b.played as SpiceCard[]).map(card=><div className="ng-spice-card" key={card.id}><span className="ng-spice-illustration"><IllustratedTile kind="century" index={merchantIllustration(card)}/></span><b><SpiceLine text={spiceCardTitle(card)}/></b></div>)}</div></section></div><div className="ng-century-hand-dock"><aside className={`ng-pocket ${pending?'ng-pocket-active':''}`} aria-label={t("你的商队")}><span>{t("你的商队")}<small>{b.cubes.reduce((sum: number, value: number) => sum + value, 0)} / 10</small></span><Cubes values={b.cubes}/>{pending&&<div className="ng-pending-actions">{b.active&&<p><SpiceLine text={spiceCardTitle(b.active)}/>{b.phase==='upgrade'&&t(` · 还可升级 ${b.remaining} 次`)}</p>}<Controls {...props} omit={['play','acquire','claim']}/></div>}</aside>{!pending&&<section className="ng-panel ng-century-hand-panel"><h3>{t("你的商队") + " "}<small>{tx(b.cubes.reduce((n: number, v: number) => n + v, 0))}{" " + t("/ 10 枚")}</small></h3><Cubes values={b.cubes}/>{!pending&&<Controls {...props} omit={['play', 'acquire', 'claim']}/>}<h3 className="ng-century-hand-heading">{t("可用商人") + " "}<small>{t(`${b.hand.length} 张`)}</small>{handOverflow&&<span className="ng-century-scroll-cue">↔ {t("滑动查看手牌")}</span>}</h3><div className="ng-market" ref={handRef} tabIndex={0} aria-label={t("可用商人")}>{b.hand.length===0&&<p className="ng-century-empty-hand">{t("商人已全部使用。休整可收回所有商人。")}</p>}{tx((b.hand as SpiceCard[]).map(c => { const onClick = choice('play', c.id); return <button type="button" aria-label={t(spiceCardTitle(c))} className="ng-spice-card ng-merchant-card" key={c.id} disabled={!onClick} onClick={onClick}><span className="ng-spice-illustration"><IllustratedTile kind="century" index={merchantIllustration(c)}/></span><b><SpiceLine text={spiceCardTitle(c)}/></b></button>; }))}</div></section>}</div>{order&&<CenturyOrderDialog order={order} cubes={b.cubes} bonus={t(orderBonus((b.goals as SpiceOrder[]).findIndex(o=>o.id===order.id)))} note={b.current!==selfID||b.phase!=="action"||view.finished?t(view.instruction):t("香料不足，凑齐后可完成订单。")} onClose={()=>setOrderID(null)} onClaim={canClaim?()=>{props.command({action:"claim",values:[order.id]});setOrderID(null);}:undefined}/>}</div>;
}
export function NewGamesTable(props: Props) { if(props.view.kind==='uno'&&props.view.finished)return <NewGameResults view={props.view} selfID={props.selfID}/>; return props.view.kind === 'sushi' ? <SushiTable {...props}/> : props.view.kind === 'uno' ? <UnoTable {...props}/> : <CenturyTable {...props}/>; }
