...
import { ArrowLeft, Eye, Check, ChevronRight, Cloud, Copy, Crown, Gamepad2, HelpCircle, History, LogOut, Maximize2, Plus, Radio, RefreshCw, RotateCcw, Settings2, Share2, Smartphone, Users, Wifi, X } from 'lucide-react';
...
type Sheet = 'profile' | 'join' | 'nearby' | 'help' | 'log' | 'menu' | 'history' | 'audience' | null;
...
export function App() {
    ...
    {tx(view && <><div className={`game-surface ${spectating?'spectator-surface':''}`} inert={!practice&&state.actionPending} aria-busy={!practice&&!!state.actionPending}>{tx(['doudizhu','guandan','mahjong'].includes(view.kind)?<ClassicTable view={view} selfID={selfID} command={run} open={openAction}/>:view.kind === 'gems' ? <GemsTable view={view} selfID={selfID} command={run} open={openAction}/> : view.kind === 'bombs' ? <BombsTable view={view} selfID={selfID} command={run} open={openAction}/> : ['sushi', 'century', 'uno'].includes(view.kind) ? <NewGamesTable view={view} selfID={selfID} command={run} open={openAction}/> : ['codenames', 'undercover'].includes(view.kind) ? <WordGamesTable key={matchKey} view={view} selfID={selfID} command={run} open={openAction}/> : <SocialTable key={practice?.id || room?.code} view={view} selfID={selfID} command={run} open={openAction}/>)}</div>{tx(!spectating&&!['doudizhu','guandan','mahjong','bombs', 'sushi', 'century', 'uno', 'codenames', 'undercover'].includes(view.kind) && <ActionDock busy={!practice&&!!state.actionPending} view={view} command={run} open={openAction}/>)}{tx(view.finished && dismissedResult !== resultKey && <div className="end-banner"><Crown size={23}/><b>{tx(typeof view.board.winner === 'string' ? view.board.winner : view.instruction)}</b>{tx(practice ? <button className="compact primary" onClick={() => startPractice(practice.kind)}>{t("\u518D\u6765\u4E00\u5C40")}</button> : host ? <button className="compact primary" onClick={() => client.replay()}>{t("\u8FD4\u56DE\u51C6\u5907")}</button> : <small>{t("\u7B49\u5F85\u623F\u4E3B\u518D\u5F00\u4E00\u5C40")}</small>)}<button className="icon dismiss-result" aria-label={t("收起结算")} onClick={() => setDismissedResult(resultKey)}><X size={18}/></button></div>)}</>)}
    ...
    {tx(sheet === 'nearby' && <Modal title={t("附近的牌桌")} onClose={() => setSheet(null)}><p className="panel-note">{t("\u6309\u7F51\u7EDC\u51FA\u53E3\u5BFB\u627E\u5019\u9009\u724C\u684C\uFF1B\u5165\u684C\u9700\u623F\u4E3B\u540C\u610F\u3002")}</p><div className="nearby-list">{tx(finding ? <div className="empty-state"><div className="spinner"/>{t("\u5BFB\u627E\u4E2D")}</div> : rooms.length ? rooms.map(r => <button key={r.code} onClick={() => join(r.code)}>{coverArt(r.kind)}<span><b>{r.hostName}{t("\u7684\u724C\u684C")}</b><small>{tx(GAMES[r.kind].name)}{t("\u00B7")}{tx(r.count)}{t("/")}{tx(r.max)}{t("\u4EBA \u00B7")}{tx(r.mode === 'lan' ? 'Wi-Fi' : '云端')}</small></span><ChevronRight size={18}/></button>) : <div className="empty-state"><Radio size={28}/><p>{t("\u9644\u8FD1\u8FD8\u6CA1\u6709\u5F00\u684C")}</p><small>{t("\u4E5F\u53EF\u4EE5\u8BA9\u670B\u53CB\u5206\u4EAB\u516D\u4F4D\u623F\u95F4\u7801")}</small></div>)}</div><footer><button className="text-button" onClick={() => setSheet('join')}>{t("\u8F93\u5165\u623F\u95F4\u7801")}</button><button className="compact" disabled={finding} onClick={() => void findRooms()}><RefreshCw size={14}/>{t("\u5237\u65B0")}</button></footer></Modal>)}
    ...
    {tx(sheet === 'menu' && <Modal title={t("牌桌菜单")} onClose={() => setSheet(null)}><div className="menu-list">{tx(view?.finished && (practice || host) && <button onClick={() => { if (practice)
        ...
        client.endGame();
        setSheet(null);
    } }}><RotateCcw size={18}/>{t("\u7ED3\u675F\u672C\u5C40\uFF0C\u8FD4\u56DE\u51C6\u5907")}</button>)}<button onClick={() => { if (spectating || !view || window.confirm(t(practice ? '结束这局同屏试玩？' : host ? '离开将关闭这张牌桌，确定？' : '请房主结束本局后再离开。')))
