...
function Modal({ title, children, onClose, wide = false, dismissible = true, className = '' }: {
    ...
    return <div className="modal-shade" onClick={onClose}><section ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label={t(title)} className={`panel app-panel ${wide ? 'wide' : ''} ${className}`} onClick={e => e.stopPropagation()}><header><h2>{tx(title)}</h2>{dismissible ? <button className="icon" aria-label={t("关闭")} onClick={onClose}><X /></button> : <LanguageToggle/>}</header><div className="panel-content" tabIndex={0} role="region" aria-label={t(title)}>{tx(children)}</div></section></div>;
}
export function App() {
