...
export function App() {
    ...
    const startPractice = (k: GameKind) => { const count = k==='doudizhu'?3:k==='guandan'||k==='mahjong'?4:k === 'werewolf' ? (wolfMode === 'judge' ? 10 : 9) : k === 'avalon' ? 5 : k === 'bombs' || k === 'undercover' ? 3 : k === 'codenames' ? 4 : 2; const players = Array.from({ length: count }, (_, i) => ({ id: `practice-${i}`, name: i === 0 ? profile!.name : (locale==='en'?['','Alex','Rain','Quinn','Sunny','Robin','Ash','Zephyr','Summer']:['','阿岚','小雨','阿七','橘子','小白','阿木','南风','夏天'])[i] || (locale==='en'?`Player ${i+1}`:`玩家${i+1}`), avatar: i === 0 ? profile!.avatar : AVATARS[i] })); setPractice({ id: crypto.randomUUID(), kind: k, players, game: modules[k].create(players, crypto.getRandomValues(new Uint32Array(1))[0], gameOptions(k, players[0].id)), viewer: players[0].id, options: gameOptions(k, players[0].id) }); setSelectedGame(null); };
