...
import { ArrowLeft, Check, ChevronRight, Cloud, Copy, Crown, Bot, HelpCircle, History, LogOut, Maximize2, Plus, Radio, RefreshCw, RotateCcw, Settings2, Share2, Smartphone, Users, Wifi, X } from 'lucide-react';
...
type Sheet = 'addSeat' | 'profile' | 'join' | 'nearby' | 'help' | 'log' | 'menu' | 'history' | null;
...
export function App() {
    ...
    {tx(sheet === 'menu' && <Modal title={t("牌桌菜单")} onClose={() => setSheet(null)}><div className="menu-list">{tx(view?.finished && (practice || host) && <button onClick={() => { if (practice)
        ...
        client.endGame();
        setSheet(null);
    } }}><RotateCcw size={18}/>{t("\u7ED3\u675F\u672C\u5C40\uFF0C\u8FD4\u56DE\u51C6\u5907")}</button>)}<button onClick={() => { if ((!view && !host) || window.confirm(t(practice ? practice.mode === 'solo' ? '结束这局单人人机？' : '结束这局同屏试玩？' : host ? '离开将关闭这张牌桌，确定？' : '请房主结束本局后再离开。')))
