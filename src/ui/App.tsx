...
function Modal({ title, children, onClose, wide = false, dismissible = true }: {
    ...
    return <div className="modal-shade" onClick={onClose}><section ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label={t(title)} className={`panel app-modal ${wide ? 'wide' : ''}`} onClick={e => e.stopPropagation()}><header><h2>{tx(title)}</h2>{dismissible ? <button className="icon" aria-label={t("关闭")} onClick={onClose}><X /></button> : <LanguageToggle/>}</header><div className="modal-body">{tx(children)}</div></section></div>;
}
export function App() {
    ...
    {tx(usableRoom && disconnected.length > 0 && <div className="presence-notice" role="status"><span>{disconnected.map(p => p.name).join('、')}{' · '}{t("\u6682\u65F6\u79BB\u7EBF \u00B7 \u5EA7\u4F4D\u5DF2\u4FDD\u7559")}</span><span>{tx(room.started ? '等待重连后继续；房主可从菜单结束本局' : '房主可移除离线座位')}</span></div>)}
    ...
    {tx(view && <><div className="game-surface" inert={!practice&&state.actionPending} aria-busy={!practice&&!!state.actionPending}>{tx(['doudizhu','guandan','mahjong'].includes(view.kind)?<ClassicTable view={view} selfID={selfID} command={run} open={openAction}/>:view.kind === 'gems' ? <GemsTable view={view} selfID={selfID} command={run} open={openAction}/> : view.kind === 'bombs' ? <BombsTable view={view} selfID={selfID} command={run} open={openAction}/> : ['sushi', 'century', 'uno'].includes(view.kind) ? <NewGamesTable view={view} selfID={selfID} command={run} open={openAction}/> : ['codenames', 'undercover'].includes(view.kind) ? <WordGamesTable key={matchKey} view={view} selfID={selfID} command={run} open={openAction}/> : <SocialTable key={practice?.id || room?.code} view={view} selfID={selfID} command={run} open={openAction}/>)}</div>{tx(!['doudizhu','guandan','mahjong','bombs', 'sushi', 'century', 'uno', 'codenames', 'undercover'].includes(view.kind) && <ActionDock busy={!practice&&!!state.actionPending} view={view} command={run} open={openAction}/>)}{tx(view.finished && dismissedResult !== resultKey && <div className="end-banner"><Crown size={23}/><b>{tx(typeof view.board.winner === 'string' ? view.board.winner : view.instruction)}</b>{tx(practice ? <button className="compact primary" onClick={() => startPractice(practice.kind)}>{t("\u518D\u6765\u4E00\u5C40")}</button> : host ? <button className="compact primary" onClick={() => client.replay()}>{t("\u8FD4\u56DE\u51C6\u5907")}</button> : <small>{t("\u7B49\u5F85\u623F\u4E3B\u518D\u5F00\u4E00\u5C40")}</small>)}<button className="icon dismiss-result" aria-label={t("收起结算")} onClick={() => setDismissedResult(resultKey)}><X size={18}/></button></div>)}</>)}
