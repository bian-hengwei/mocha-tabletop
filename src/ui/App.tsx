...
function Modal({ title, children, onClose, wide = false }: {
    ...
    useEffect(() => { const previous = document.activeElement as HTMLElement | null, node = panel.current!; if (!node.contains(document.activeElement))
        node.querySelector<HTMLElement>('input,button,select,[tabindex="0"]')?.focus(); const handle = (e: KeyboardEvent) => { if (e.key === 'Escape') {
        e.stopPropagation();
        close.current();
    } if (e.key === 'Tab') {
        const targets = [...node.querySelectorAll<HTMLElement>('button:not(:disabled),input:not(:disabled),select:not(:disabled),a[href],[tabindex="0"]')].filter(el => el.getClientRects().length);
    ...
    } }; node.addEventListener('keydown', handle); return () => { node.removeEventListener('keydown', handle); if (previous?.isConnected)
        previous.focus(); }; }, []);
    return <div className="modal-shade" onClick={onClose}><section ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label={t(title)} className={`panel ${wide ? 'wide' : ''}`} onClick={e => e.stopPropagation()}><header><h2>{tx(title)}</h2><button className="icon" aria-label={t("关闭")} onClick={onClose}><X /></button></header>{tx(children)}</section></div>;
}
export function App() {
    ...
    const gameOptions = (k: GameKind, hostID?: string): GameOptions | undefined => k === 'werewolf' ? { language: locale, werewolfMode: wolfMode, werewolfPreset:ruleOptions.werewolfPreset,werewolfWin:ruleOptions.werewolfWin, ...(hostID && wolfMode !== 'standard' ? { moderatorID: hostID } : {}) } : { language: locale,...(k==='uno'?{unoMode:ruleOptions.unoMode,unoChallenge:ruleOptions.unoChallenge}:{}) };
    ...
    const startPractice = (k: GameKind) => { const count = k === 'werewolf' ? (wolfMode === 'judge' ? 10 : 9) : k === 'avalon' ? 5 : k === 'bombs' || k === 'undercover' ? 3 : k === 'codenames' ? 4 : 2; const players = Array.from({ length: count }, (_, i) => ({ id: `practice-${i}`, name: i === 0 ? profile!.name : (locale==='en'?['','Alex','Rain','Quinn','Sunny','Robin','Ash','Zephyr','Summer']:['','阿岚','小雨','阿七','橘子','小白','阿木','南风','夏天'])[i] || (locale==='en'?`Player ${i+1}`:`玩家${i+1}`), avatar: i === 0 ? profile!.avatar : AVATARS[i] })); setPractice({ id: crypto.randomUUID(), kind: k, players, game: modules[k].create(players, crypto.getRandomValues(new Uint32Array(1))[0], gameOptions(k, players[0].id)), viewer: players[0].id, options: gameOptions(k, players[0].id) }); setSelectedGame(null); };
    ...
    const share = async () => { const url = state.inviteURL || `${location.origin}/?room=${room?.code}`; try {
        if (navigator.share)
            await navigator.share({ title: 'Mocha 桌游', text: `来玩${GAMES[room!.kind].name}`, url });
        else {
    ...
    {tx(view && <><div className="game-surface">{tx(view.kind === 'gems' ? <GemsTable view={view} selfID={selfID} command={run} open={openAction}/> : view.kind === 'bombs' ? <BombsTable view={view} selfID={selfID} command={run} open={openAction}/> : ['sushi', 'century', 'uno'].includes(view.kind) ? <NewGamesTable view={view} selfID={selfID} command={run} open={openAction}/> : ['codenames', 'undercover'].includes(view.kind) ? <WordGamesTable view={view} selfID={selfID} command={run} open={openAction}/> : <SocialTable key={practice?.id || room?.code} view={view} selfID={selfID} command={run} open={openAction}/>)}</div>{tx(!['bombs', 'sushi', 'century', 'uno', 'codenames', 'undercover'].includes(view.kind) && <ActionDock view={view} command={run} open={openAction}/>)}{tx(view.finished && dismissedResult !== resultKey && <div className="end-banner"><Crown size={23}/><b>{tx(typeof view.board.winner === 'string' ? view.board.winner : view.instruction)}</b>{tx(practice ? <button className="compact primary" onClick={() => startPractice(practice.kind)}>{t("\u518D\u6765\u4E00\u5C40")}</button> : host ? <button className="compact primary" onClick={() => client.replay()}>{t("\u8FD4\u56DE\u51C6\u5907")}</button> : <small>{t("\u7B49\u5F85\u623F\u4E3B\u518D\u5F00\u4E00\u5C40")}</small>)}<button className="icon dismiss-result" aria-label={t("收起结算")} onClick={() => setDismissedResult(resultKey)}><X size={18}/></button></div>)}</>)}
    ...
    {tx(sheet === 'nearby' && <Modal title={t("附近的牌桌")} onClose={() => setSheet(null)}><p className="panel-note">{t("\u6309\u7F51\u7EDC\u51FA\u53E3\u5BFB\u627E\u5019\u9009\u724C\u684C\uFF1B\u5165\u684C\u9700\u623F\u4E3B\u540C\u610F\u3002")}</p><div className="nearby-list">{tx(finding ? <div className="empty-state"><div className="spinner"/>{t("\u5BFB\u627E\u4E2D")}</div> : rooms.length ? rooms.map(r => <button key={r.code} onClick={() => join(r.code)}><img src={cover(r.kind)} alt=""/><span><b>{tx(r.hostName)}{t("\u7684\u724C\u684C")}</b><small>{tx(GAMES[r.kind].name)}{t("\u00B7")}{tx(r.count)}{t("/")}{tx(r.max)}{t("\u4EBA \u00B7")}{tx(r.mode === 'lan' ? 'Wi-Fi' : '云端')}</small></span><ChevronRight size={18}/></button>) : <div className="empty-state"><Radio size={28}/><p>{t("\u9644\u8FD1\u8FD8\u6CA1\u6709\u5F00\u684C")}</p><small>{t("\u4E5F\u53EF\u4EE5\u8BA9\u670B\u53CB\u5206\u4EAB\u516D\u4F4D\u623F\u95F4\u7801")}</small></div>)}</div><footer><button className="text-button" onClick={() => setSheet('join')}>{t("\u8F93\u5165\u623F\u95F4\u7801")}</button><button className="compact" disabled={finding} onClick={() => void findRooms()}><RefreshCw size={14}/>{t("\u5237\u65B0")}</button></footer></Modal>)}
