...
export function App() {
    ...
    {tx(view && <><div className="game-surface" inert={!practice&&state.actionPending} aria-busy={!practice&&!!state.actionPending}>{tx(['doudizhu','guandan','mahjong'].includes(view.kind)?<ClassicTable view={view} selfID={selfID} command={run} open={openAction}/>:view.kind === 'gems' ? <GemsTable view={view} selfID={selfID} command={run} open={openAction}/> : view.kind === 'bombs' ? <BombsTable view={view} selfID={selfID} command={run} open={openAction}/> : ['sushi', 'century', 'uno'].includes(view.kind) ? <NewGamesTable view={view} selfID={selfID} command={run} open={openAction} onShowResults={()=>setDismissedResult(null)}/> : ['codenames', 'undercover'].includes(view.kind) ? <WordGamesTable key={matchKey} view={view} selfID={selfID} command={run} open={openAction}/> : <SocialTable key={practice?.id || room?.code} view={view} selfID={selfID} command={run} open={openAction}/>)}</div>{tx(!['doudizhu','guandan','mahjong','bombs', 'sushi', 'century', 'uno', 'codenames', 'undercover'].includes(view.kind) && <ActionDock busy={!practice&&!!state.actionPending} view={view} command={run} open={openAction}/>)}{tx(view.finished && dismissedResult !== resultKey && <div className={`end-banner ${view.kind==='sushi'?'end-banner-sushi':''}`}><Crown size={23}/><b>{tx(typeof view.board.winner === 'string' ? view.board.winner : view.instruction)}</b>{view.kind==='sushi'&&<SushiResults players={view.board.players} roundScores={view.board.roundScores} winners={view.board.winners} selfID={selfID}/>} {tx(practice ? <button className="compact primary" onClick={() => startPractice(practice.kind)}>{t("\u518D\u6765\u4E00\u5C40")}</button> : host ? <button className="compact primary" onClick={() => client.replay()}>{t("\u8FD4\u56DE\u51C6\u5907")}</button> : <small>{t("\u7B49\u5F85\u623F\u4E3B\u518D\u5F00\u4E00\u5C40")}</small>)}<button className="icon dismiss-result" aria-label={t("收起结算")} onClick={() => setDismissedResult(resultKey)}><X size={18}/></button></div>)}</>)}
