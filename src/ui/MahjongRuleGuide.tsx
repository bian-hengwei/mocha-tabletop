...
import {mahjongModeNotes,mahjongVariantGuide} from '../i18n/mahjongVariants';
...
export function MahjongRuleGuide({mode='guangdong',legacy=false}:{mode?:MahjongMode;legacy?:boolean}){
 ...
 return <div className="rule-guide mahjong-guide">
  ...
  <div id={`${id}-content`} role="tabpanel" aria-labelledby={`${id}-tab-${tab}`} tabIndex={0}>
   {tab===2&&!legacy&&<label className="mahjong-guide-mode">{locale==='zh'?'选择模式':'Choose a variant'}<select aria-label={locale==='zh'?'查看模式规则':'View variant rules'} value={selected} onChange={e=>setSelected(e.target.value as MahjongMode)}>{Object.entries(MAHJONG_MODES).map(([key,name])=><option key={key} value={key}>{t(name)}</option>)}</select><small>{mahjongModeNotes[selected][locale==='zh'?0:1]}</small></label>}
   ...
   <p className="rule-edition">{guide.edition}</p>
