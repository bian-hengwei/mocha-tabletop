...
import {tileTitle,type Tile,type Meld,type MahjongMode,type MahjongAssistance} from '../core/games/mahjong';
...
type Board={assistance?:MahjongAssistance|null;hand:Tile[];players:Seat[];current:string;phase:string;mode:MahjongMode;wallCount:number;drawn:string|null;wildValue:number;turn:number;pending:{tile:Tile;from:number;rob:boolean}|null;wins:{player:number;tile:Tile;points:number;selfDraw:boolean}[]};
 ...
 return <div className={`classic-table mj-table game-table-mahjong ${view.spectating?'mj-spectating':''} ${view.finished?'mj-finished':''}`} onKeyDown={e=>{if(e.key==='Escape'&&!historyOpen&&riverSeat===null)clear();}}>
  ...
  {!view.spectating&&<section className="mj-hand-panel">
   <div className="mj-hand-toolbar"><div className="mj-status" aria-live="polite">{t(instruction)}</div><div className="mj-actions">
    ...
    {b.assistance&&<MahjongAssist key={selfID} data={b.assistance} selected={selected[0]}/>}
   </div></div>
