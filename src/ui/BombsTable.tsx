...
const titles: Record<BombKind, string> = { bomb: '闹闹牌', defuse: '安抚', attack: '加班', skip: '跳过', favor: '借一张', shuffle: '洗牌', future: '偷瞄三张', nope: '等等', moonCat: '月亮猫', cloudCat: '云朵猫', leafCat: '叶子猫', starCat: '星星猫', sunCat: '太阳猫' };
const details: Record<BombKind, string> = { bomb: '安抚，或告别这张桌子', defuse: '抽到闹闹牌时使用', attack: '剩余加班回合 +2，交给下家', skip: '结束一个回合，不抽牌', favor: '选一位玩家，让对方交一张牌', shuffle: '打乱抽牌堆，继续出牌', future: '秘密看顶部三张，不改变顺序', nope: '取消效果；再次等等则恢复', moonCat: '两张随机拿牌 · 三张指定牌名', cloudCat: '两张随机拿牌 · 三张指定牌名', leafCat: '两张随机拿牌 · 三张指定牌名', starCat: '两张随机拿牌 · 三张指定牌名', sunCat: '两张随机拿牌 · 三张指定牌名' };
...
export function BombsIllustratedCard({ card, selected = false, onClick, small = false, order }: {
    ...
    return <button type="button" disabled={!onClick} className={`bt-card ${small ? 'bt-small' : ''} ${selected ? 'bt-selected' : ''}`} style={{ '--card-accent': accents[card.kind] } as CSSProperties} aria-label={tx(card.title)} aria-pressed={onClick ? selected : undefined} onClick={onClick}>
    <span className="bt-card-heading"><b>{tx(card.title)}</b><KindIcon kind={card.kind} size={13}/></span><CatArt kind={card.kind}/><span className="bt-card-foot">{tx(short[card.kind])}</span>{tx(selected && <span className="bt-card-check"><Check size={13}/></span>)}{tx(order !== undefined && <span className="bt-order">{tx(order)}</span>)}
  </button>;
}
export function BombsTable({ view, selfID, command }: Props) {
    ...
    const actionTitle = single?.id === 'give' ? '交给对方' : single?.id === 'nope' ? (response?.cancelled ? '恢复效果' : '等等这张') : single ? '打出这张' : validCombo ? (cards.length === 2 ? '随机拿一张' : '指定牌名') : '';
    const play = () => {
    ...
    if (view.finished)
        ...
        focus = <div className="bt-focus"><span className="bt-eyebrow">{t("选择目标")}</span><h2>{t("点一位朋友的头像")}</h2><p>{t("目标确定后，大家仍可等等。")}</p><button className="bt-quiet" onClick={() => run('cancel')}>{t("取消出牌")}</button></div>;
    else if (request)
        ...
        focus = <div className="bt-focus bt-response"><span className="bt-eyebrow">{displayName(response.actor)}{" " + t("出牌")}</span><h2>{tx(response.cards.length > 1 ? `${response.cards.length} 张同名组合` : response.cards[0].title)}{tx(response.target && <small>{" " + t("→") + " "}{displayName(response.target)}</small>)}</h2>{tx(response.requested && <p>{t("索要 ·") + " "}{tx(titles[response.requested as BombKind])}</p>)}<div className="bt-response-status">{tx(b.players.filter((p: any) => p.alive).map((p: any) => <span key={p.id} className={response.passed.includes(p.id) ? 'confirmed' : ''} aria-label={tx(`${p.name}${response.passed.includes(p.id) ? '已确认' : '待响应'}`)}>{tx(p.avatar)}{tx(response.passed.includes(p.id) && <Check size={10}/>)}</span>))}<b>{tx(response.cancelled ? '已等等，可再次恢复' : '等待确认')}</b></div><div className="bt-action-row">{tx(single?.id === 'nope' && <button className="bt-primary" onClick={play}><Hand size={15}/>{tx(actionTitle)}</button>)}{tx(get('pass') && <button className="bt-secondary" onClick={() => run('pass')}>{tx(response.cancelled ? '保持等等' : '不等等')}<Check size={15}/></button>)}</div>{tx(get('nope') && !single && <p className="bt-hint">{t("点手里的等等牌，可以改变结果。")}</p>)}</div>;
    else if (give)
        focus = <div className="bt-focus"><span className="bt-eyebrow">{t("交牌给") + " "}{displayName(b.give.actor)}</span><h2>{tx(first ? first.title : '你决定交哪张')}</h2><p>{t("只把这一张交给对方。")}</p>{tx(single && <button className="bt-primary" onClick={play}>{t("交出这张")}<ArrowRight size={15}/></button>)}</div>;
        ...
        focus = <div className="bt-focus bt-danger"><span className="bt-eyebrow">{t("抽到闹闹牌！")}</span><h2>{t("这只猫需要安抚？")}</h2><div className="bt-action-row"><button className="bt-primary" onClick={() => run('defuse')}><Scissors size={18}/>{t("使用安抚")}</button><button className="bt-quiet" onClick={() => setExplosionPrompt(true)}>{t("放弃安抚")}</button></div></div>;
    else if (get('insert'))
        focus = <div className="bt-focus bt-insert"><span className="bt-eyebrow">{t("秘密放回闹闹牌")}</span><h2>{tx(insertion === 0 ? '下一张就是它' : insertion === b.deckCount ? '藏到最底下' : `放在第 ${insertion + 1} 张`)}</h2><div className="bt-insert-range"><button aria-label={t("闹闹牌移向顶部")} disabled={insertion === 0} onClick={() => setInsertion(n => n - 1)}>{t("−")}</button><input aria-label={t("闹闹牌插入位置")} type="range" min={0} max={b.deckCount} value={insertion} onChange={e => setInsertion(Number(e.target.value))}/><button aria-label={t("闹闹牌移向底部")} disabled={insertion === b.deckCount} onClick={() => setInsertion(n => n + 1)}>{t("＋")}</button></div><div className="bt-action-row"><button className="bt-quiet" onClick={() => setInsertion(0)}>{t("顶部")}</button><button className="bt-quiet" onClick={() => setInsertion(b.deckCount)}>{t("底部")}</button><button className="bt-primary" onClick={() => run('insert', [String(insertion)])}>{t("放好了")}<Check size={14}/></button></div></div>;
    else if (first)
        focus = <div className="bt-focus"><span className="bt-eyebrow">{tx(cards.length > 1 ? `已选 ${cards.length} 张` : '选中的手牌')}</span><h2>{tx(cards.length > 1 && cards.every(c => c.kind === first.kind) ? `${first.title} ×${cards.length}` : first.title)}</h2><p>{tx(cards.length > 1 && !cards.every(c => c.kind === first.kind) ? '组合需要同名牌' : details[first.kind])}</p><div className="bt-action-row">{tx(actionTitle && <button className="bt-primary" onClick={play}>{tx(actionTitle)}<ArrowRight size={15}/></button>)}<button className="bt-quiet" onClick={() => setSelected([])}>{t("收起")}</button></div></div>;
    else
        focus = <div className="bt-focus"><span className="bt-eyebrow">{tx(mine ? '你的回合' : '轮到')}</span><h2>{tx(mine ? '要出哪张？' : displayName(b.current))}</h2><p>{tx(mine ? '轻点手牌，或从牌堆抽一张。' : b.phase === 'give' ? `${displayName(b.give.target)} 正在挑选手牌` : b.phase === 'insert' ? '正在秘密放回闹闹牌' : '看看手里的牌，等下一次机会。')}</p>{tx(b.turnsRemaining > 1 && <span className="bt-debt"><Zap size={14}/>{" " + t("还需结束") + " "}{tx(b.turnsRemaining)}{" " + t("个回合")}</span>)}</div>;
