...
import {combinationKey,legalPokerCombinations,pokerTitle,rankName,type PokerCard,type Combination} from '../core/games/poker';
...
export function ClassicTable({view,selfID,command,open}:Props){
 ...
 return <div className={`classic-table ${isMahjong?'mahjong-table':'poker-table'} game-table-${view.kind} ${isEnd?'is-finished':''}`}>
  ...
  {!isEnd&&<><div className="classic-arena">
   ...
   {!isMahjong&&<section className="classic-felt" aria-label={t('出牌区')}>
    ...
    <div className={`poker-play-area ${b.last?'has-play play-'+position(b.last.player):''}`}>{b.last?<><p className="played-caption">{b.players[b.last.player].name} <span>{combinationLabel(b.last.combo)}</span></p><div className="played-scroll" tabIndex={b.last.cards.length>8?0:undefined} aria-label={t('出牌区')}><div className="classic-cards played">{b.last.cards.map((c:PokerCard)=><Face key={c.id} card={c} mahjong={false} mini/>)}</div></div></>:<p className="table-prompt">{t(waitingForPlay?instruction:b.phase==='bid'?'每人依次叫 1–3 分或不叫':'自由领出，选择一组手牌')}</p>}</div>
  ...
  <section className="classic-hand-panel">
   <header><h3><span className="hand-avatar">{me.avatar}</span>{t('我的手牌')} <small>{hand.length}</small><span className="hand-score">{me.score} {t('分')}</span></h3>{(selectAction?.id==='exchange'||selectAction?.id==='return')&&<span className="hand-gesture">{t(selectAction.id==='exchange'?'同花色三张':'选择还贡牌')}</span>}</header>
   ...
   <div className="classic-action-panel"><div className={`classic-selection ${view.actions.length?'actionable':''}`} aria-live="polite">{selected.length?`${t('已选')} ${selected.length} · ${t(combo?.type||(valid?'可以确认':'请调整选牌'))}`:t(isMahjong&&me?.won&&b.mode==='bloodflow'?'已胡牌：只打新摸牌':instruction)}</div>
    {selectAction?.id==='play'&&combinations.length>1&&<label className="classic-declare">{t('出牌牌型')}<select aria-label={t('出牌牌型')} value={combo?combinationKey(combo):''} onChange={e=>setDeclaration(e.target.value)}>{combinations.map(c=><option key={combinationKey(c)} value={combinationKey(c)}>{combinationLabel(c)}</option>)}</select></label>}
  ...
  {isEnd&&<section className="classic-results"><header><h3>{t(view.finished?'本局结束':'本轮结算')}</h3>{view.kind==='guandan'&&<p>{t('两队级数')} <b>{rankName(b.levels[0]===2?15:b.levels[0])}</b> · <b>{rankName(b.levels[1]===2?15:b.levels[1])}</b></p>}</header>{view.finished&&<p className="classic-result-outcome">{t(view.instruction)}</p>}<div className="classic-result-grid">{b.players.map((p:{id:string;name:string;avatar:string;score:number;team?:number|null;rank?:number;won?:boolean;hand?:(Tile|PokerCard)[]})=><div className="classic-result-player" key={p.id}><p><span>{p.avatar} {p.name}</span><strong>{p.score>0?'+':''}{p.score} {t('分')}</strong></p>{(p.team!=null||p.rank||p.won)&&<small>{[p.team!=null?`${t('队伍')} ${p.team+1}`:'',p.rank?`${t('名次')} ${p.rank}`:p.won?t('已胡牌'):''].filter(Boolean).join(' · ')}</small>}{p.hand?.length?<div className="classic-reveal" tabIndex={0} aria-label={`${p.name} · ${t('手牌可左右滑动')}`}>{p.hand.map(c=><Face key={c.id} card={c} mahjong={isMahjong} mini/>)}</div>:null}</div>)}</div><footer className="classic-result-controls">{view.actions.map(a=><button key={a.id} className="compact primary" onClick={()=>a.choices.length?open(a):command({action:a.id,values:[]})}>{t(a.title)}</button>)}{isMahjong&&b.wins.length>0&&<button className="compact history-button" aria-label={`${t('胡牌记录')} (${b.wins.length})`} onClick={()=>setHistoryOpen(true)}><History size={15}/><span>{b.wins.length}</span></button>}</footer></section>}
