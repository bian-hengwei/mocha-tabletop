...
export function ClassicTable({view,selfID,command,open,onReplay,replayLabel}:Props){
 ...
 return <div className={`classic-table ${isMahjong?'mahjong-table':'poker-table'} game-table-${view.kind} ${view.finished||b.phase==='roundEnd'?'is-finished':''}`}>
  ...
  <section className="classic-hand-panel">
   <header><h3><span className="hand-avatar">{me.avatar}</span>{t('我的手牌')} <small>{hand.length}</small><span className="hand-score">{me.score} {t('分')}</span></h3>{(selectAction?.id==='exchange'||selectAction?.id==='return')?<span className="hand-gesture">{t(selectAction.id==='exchange'?'同花色三张':'选择还贡牌')}</span>:hiddenHand&&<span className="hand-gesture hand-scroll-cue"><MoveHorizontal size={13} aria-hidden="true"/>{t('滑动查看手牌')}</span>}</header>
