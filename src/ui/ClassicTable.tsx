...
export function ClassicTable({view,selfID,command,open,onReplay,replayLabel}:Props){
 ...
 return <div className={`classic-table ${isMahjong?'mahjong-table':'poker-table'} game-table-${view.kind} ${view.finished||b.phase==='roundEnd'?'is-finished':''}`}>
  ...
  <section className="classic-hand-panel">
   ...
   <div className="classic-action-panel"><div className={`classic-selection ${view.actions.length?'actionable':''}`} aria-live="polite">{invalidPlay?t(invalidPlay):selected.length?`${t('已选')} ${selected.length} · ${t(combo?.type||(valid?'可以确认':'请调整选牌'))}`:t(isMahjong&&me?.won&&b.mode==='bloodflow'?'已胡牌：只打新摸牌':view.instruction)}</div>
