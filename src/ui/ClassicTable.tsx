...
export function ClassicTable({view,selfID,command,open}:Props){
 ...
 const valid=!!selectAction&&selected.length>=selectAction.min&&selected.length<=selectAction.max&&selected.every(id=>selectAction.choices.some(c=>c.id===id))&&(selectAction.id!=='play'||!!combo)&&(selectAction.id!=='exchange'||b.exchangeRule==='any'||b.exchangeRule==='anyThree'||new Set((chosen as Tile[]).map(c=>Math.floor(c.value/9))).size===1);
 ...
 return <div className={`classic-table ${isMahjong?'mahjong-table':'poker-table'} game-table-${view.kind} ${view.finished||b.phase==='roundEnd'?'is-finished':''}`}>
  <header className="classic-edition"><span><i/>{t(isMahjong?MAHJONG_MODES[b.mode as keyof typeof MAHJONG_MODES]:view.kind==='guandan'?'双副牌 · 对家合作':'经典叫分 · 三人局')}</span>{isMahjong&&b.wildValue>=0&&<span className="mahjong-wild-indicator" title={b.indicator?`${t('指示牌')} · ${t(tileTitle(b.indicator))}`:undefined}>{t('鬼牌')}<Face card={{id:'wild-indicator',value:b.wildValue}} mahjong mini/></span>}<strong>{isMahjong?`${t('余牌')} ${b.wallCount}`:view.kind==='guandan'?`${t('级牌')} ${rankName(b.level===2?15:b.level)} · ${t('轮次')} ${b.round}`:`${t('底分')} ${b.bid} · ×${b.multiplier}`}</strong></header>
  ...
  <section className="classic-hand-panel">
   <header><h3><span className="hand-avatar">{me.avatar}</span>{t('我的手牌')} <small>{hand.length}</small><span className="hand-score">{me.score} {t('分')}</span></h3><span className="hand-gesture">{t(selectAction?.id==='exchange'?(b.exchangeRule==='any'?'任意张数':b.exchangeRule==='anyThree'?'任意三张':'同花色三张'):selectAction?.id==='return'?'选择还贡牌':isMahjong&&selectAction?.id==='discard'?'选牌后确认，或双击打出':'选牌后确认')}<small>{t('手牌可左右滑动')}</small></span></header>
   ...
   <div className="classic-action-panel"><div className={`classic-selection ${view.actions.length?'actionable':''}`} aria-live="polite">{selected.length?`${t('已选')} ${selected.length} · ${t(combo?.type||(valid?'可以确认':'请调整选牌'))}`:t(isMahjong&&me?.won&&(b.locked||b.mode==='bloodflow')?'已胡牌：只打新摸牌':view.instruction)}</div>
