...
export function ClassicTable({view,selfID,command,open}:Props){
 ...
 const rows=hand.length>10?2:1,columns=Math.ceil(hand.length/rows);
 ...
 return <div className={`classic-table ${isMahjong?'mahjong-table':'poker-table'} ${view.finished||b.phase==='roundEnd'?'is-finished':''}`}>
  ...
  <div className="classic-arena">
   ...
   {!isMahjong&&<section className="classic-felt" aria-label={t('出牌区')}>
    ...
    <div className={`poker-play-area ${b.last?'has-play':''}`}>{b.last?<><p className="played-caption">{b.players[b.last.player].name} <span>{t(b.last.combo.type)}</span></p><div className="played-scroll" tabIndex={b.last.cards.length>8?0:undefined} aria-label={t('出牌区')}><div className="classic-cards played">{b.last.cards.map((c:PokerCard)=><Face key={c.id} card={c} mahjong={false} mini/>)}</div></div></>:<p className="table-prompt">{t(b.phase==='bid'?'每人依次叫 1–3 分或不叫':'自由领出，选择一组手牌')}</p>}</div>
   ...
   {isMahjong&&<section className="mahjong-public" aria-label={t('公共牌池')}>
    <div className="mahjong-center"><div className="mahjong-compass" aria-hidden="true"><span>N</span><span>W</span><b>M</b><span>E</span><span>S</span></div>{b.pending?<div className="mahjong-latest"><small>{b.players[b.pending.from].name}</small><Face card={b.pending.tile} mahjong/><b>{t(b.pending.rob?'抢杠响应':'最新打出')}</b></div>:<span className="mahjong-center-mark">MOCHA<br/>MAHJONG</span>}</div>
    <div className="mahjong-rivers">{b.players.map((p:any,i:number)=><section key={p.id} className={`river-seat river-${position(i)}`}><header><b title={p.name}>{p.name}</b><span>{t('牌河')} {p.discards.length}</span></header><div className="mahjong-melds">{p.melds.map((m:any,j:number)=><div key={j} aria-label={t(m.type==='concealed'?'暗杠':'副露')}>{m.tiles.length?m.tiles.map((tile:Tile)=><Face key={tile.id} card={tile} mahjong mini/>):[0,1,2,3].map(k=><Back key={k} mahjong/>)}</div>)}</div><div className="classic-cards river" tabIndex={p.discards.length?0:undefined} aria-label={`${p.name} · ${t('牌河')}`}>{p.discards.map((tile:Tile)=><Face key={tile.id} card={tile} mahjong mini/>)}</div></section>)}</div>
   </section>}
  </div>
