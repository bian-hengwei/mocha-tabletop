...
export function GemsTable({ view, selfID, command, open }: Props) {
    ...
    const takeOptionsLabel = distinct ? `${t('选择宝石')}: ${t(`${distinct.min} 种颜色`)}${pair ? ` · ${t('或同色 2 枚')}` : ''}` : undefined;
    ...
    const inspectedAvailable = card && [...b.market, ...b.hand].some((item: { id: string }) => item.id === card.id);
    const cardNote = !card || actFor('buy', card) ? ''
        : b.payment?.card.id === card.id ? '在底部确认支付，或取消购买'
        : view.finished ? '本局已结束'
        : b.current !== selfID ? '等待你的回合'
        : b.phase !== 'action' ? '请先完成当前操作'
        : inspectedAvailable ? '筹码不足，暂不能购买。永久奖励和黄金已计入。'
        : '这张牌不在市场或你的预留中';
