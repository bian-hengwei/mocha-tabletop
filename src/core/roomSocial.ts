...
export interface RoomReaction {id:string;playerID:string;reaction:ReactionID;at:number}
...
export function applyRoomSocial(state:SocialState,room:RoomInfo,actor:string,input:unknown,requestID:unknown,now:number):SocialState {
 ...
 if(command.type==='chat'){
  ...
  if(Object.keys(command).some(k=>!['type','reaction'].includes(k))||!REACTIONS.includes(command.reaction as ReactionID))throw new Error('请选择一个表情');
 ...
 return {
  ...
  reactions:[...state.reactions.filter(r=>r.at+SOCIAL_DURATION>now&&(!reaction||r.playerID!==actor)),...(reaction?[{id,playerID:actor,reaction,at:now}]:[])],
