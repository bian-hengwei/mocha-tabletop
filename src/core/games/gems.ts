...
export const gems:GameModule<GemsState>={
  ...
  view(s,playerID,spectator=false){if(spectator)playerID='';
    ...
    if(viewer<0&&!spectator)return{kind:'gems',phase:'不在本局',instruction:'仅本局玩家可查看',finished:s.finished,actions:[],sections:[],log:[],board:{}};
...
// Numerical component facts from the verified base-game catalog. No commercial illustrations or rulebook text.
