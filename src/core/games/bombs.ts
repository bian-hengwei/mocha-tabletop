...
const HELP:Record<BombKind,string>={bomb:'没有拆弹牌就出局',defuse:'拆弹爆炸牌，并秘密放回牌堆',attack:'剩余攻击回合再加 2，交给下家',skip:'不抽牌，结束一个回合',favor:'对方选择一张手牌交给你',shuffle:'打乱抽牌堆',future:'秘密查看顶部 3 张',nope:'暂停当前效果，或撤回暂停',moonCat:'同名对子随机拿牌，三张指定牌名',cloudCat:'同名对子随机拿牌，三张指定牌名',leafCat:'同名对子随机拿牌，三张指定牌名',starCat:'同名对子随机拿牌，三张指定牌名',sunCat:'同名对子随机拿牌，三张指定牌名'};
...
function resolve(s:BombsState,e:BombEffect){
  s.phase={kind:'turn'};
  if(e.cancelled){record(s,'效果已暂停');return;}
...
export const bombs:GameModule<BombsState>={
  ...
  view(s,playerID){
    ...
    if(!finished(s))switch(phase.kind){
      ...
      case 'response':{const e=phase.effect;label='响应窗口';instruction=`${name(s,e.actor)} · ${effectTitle(e)}${e.cancelled?' · 已暂停':''}`;board.response={actor:e.actor,cards:e.cards,target:e.target??null,requested:e.requested??null,cancelled:e.cancelled,passed:e.passed};sections.push({id:'responses',title:'响应',items:s.alive.map(id=>({id,title:name(s,id),detail:e.passed.includes(id)?'已确认':'待响应'}))});if(s.alive.includes(playerID)){if(!e.passed.includes(playerID))actions.push(action('pass','继续'));const nopes=hand.filter(c=>c.kind==='nope');if(nopes.length)actions.push(action('nope',e.cancelled?'恢复效果':'否决',choices(nopes),1,1));}break;}
