...
const HELP:Record<BombKind,string>={bomb:'没有安抚牌就出局',defuse:'安抚闹闹牌，并秘密放回牌堆',attack:'剩余加班回合再加 2，交给下家',skip:'不抽牌，结束一个回合',favor:'对方选择一张手牌交给你',shuffle:'打乱抽牌堆',future:'秘密查看顶部 3 张',nope:'暂停当前效果，或撤回暂停',moonCat:'同名对子随机拿牌，三张指定牌名',cloudCat:'同名对子随机拿牌，三张指定牌名',leafCat:'同名对子随机拿牌，三张指定牌名',starCat:'同名对子随机拿牌，三张指定牌名',sunCat:'同名对子随机拿牌，三张指定牌名'};
...
export const bombs:GameModule<BombsState>={
  create(players,seed){
    ...
    const s:BombsState={players:structuredClone(players),hands:Object.fromEntries(players.map(p=>[p.id,[]])),deck:[],discard:[],eliminatedCards:[],alive:players.map(p=>p.id),current:players[0].id,turnsRemaining:1,attacked:false,phase:{kind:'turn'},history:['每人 7 张普通牌和 1 张安抚牌'],rng:seed>>>0,serial:0};
  ...
  view(s,playerID){
    ...
    if(!finished(s))switch(phase.kind){
      ...
      case 'response':{const e=phase.effect;label='响应窗口';instruction=`${name(s,e.actor)} · ${effectTitle(e)}${e.cancelled?' · 已暂停':''}`;board.response={actor:e.actor,cards:e.cards,target:e.target??null,requested:e.requested??null,cancelled:e.cancelled,passed:e.passed};sections.push({id:'responses',title:'响应',items:s.alive.map(id=>({id,title:name(s,id),detail:e.passed.includes(id)?'已确认':'待响应'}))});if(s.alive.includes(playerID)){if(!e.passed.includes(playerID))actions.push(action('pass','继续'));const nopes=hand.filter(c=>c.kind==='nope');if(nopes.length)actions.push(action('nope',e.cancelled?'恢复效果':'等等',choices(nopes),1,1));}break;}
      ...
      case 'future':label='偷瞄牌堆';instruction=mine?'顶部三张 · 仅你可见':`等待 ${name(s,s.current)}`;if(mine){board.privateFuture=phase.cards.map((c,i)=>({...c,id:`future-${i}`}));sections.push({id:'future',title:'牌堆顶',private:true,items:phase.cards.map((c,i)=>({id:`future-${i}`,title:`${i+1} · ${c.title}`}))});actions.push(action('done','继续'));}break;
      case 'bomb':label='抽到闹闹牌';instruction=`${name(s,s.current)} 抽到了闹闹牌`;if(mine)actions.push(action('defuse','安抚'),action('explode','放弃安抚'));break;
      case 'insert':label='秘密放回';instruction=mine?'选择闹闹牌位置':`等待 ${name(s,s.current)} 放回闹闹牌`;if(mine)actions.push(action('insert','放回闹闹牌',Array.from({length:s.deck.length+1},(_,i)=>({id:String(i),title:i===0?'最上面':i===s.deck.length?'最底下':`第 ${i+1} 张`})),1,1));break;
    }
  ...
  apply(state,playerID,command){
    ...
    switch(command.action){
      case 'draw':{if(!s.deck.length)throw Error('牌堆为空');const c=s.deck.shift()!;if(c.kind==='bomb'){record(s,`${name(s,s.current)} 抽到了闹闹牌`);if(s.hands[s.current].some(c=>c.kind==='defuse'))s.phase={kind:'bomb',card:c};else explode(s,c);}else{s.hands[s.current].push(c);record(s,`${name(s,s.current)} 抽了一张牌`);endTurn(s);}break;}
      ...
      case 'nope':{if(p.kind!=='response')throw Error('阶段无效');const i=s.hands[playerID].findIndex(c=>c.id===v[0]);s.discard.push(s.hands[playerID].splice(i,1)[0]);p.effect.cancelled=!p.effect.cancelled;p.effect.passed=[];record(s,`${name(s,playerID)} 等等 · ${p.effect.cancelled?'取消':'恢复'}效果`);break;}
      ...
      case 'defuse':{if(p.kind!=='bomb')throw Error('阶段无效');const i=s.hands[s.current].findIndex(c=>c.kind==='defuse');if(i<0)throw Error('没有安抚牌');s.discard.push(s.hands[s.current].splice(i,1)[0]);s.phase={kind:'insert',card:p.card};record(s,`${name(s,s.current)} 使用安抚牌`);break;}
      case 'explode':if(p.kind!=='bomb')throw Error('阶段无效');explode(s,p.card);break;
      case 'insert':if(p.kind!=='insert')throw Error('阶段无效');s.deck.splice(Number(v[0]),0,p.card);record(s,`${name(s,s.current)} 已秘密放回闹闹牌`);endTurn(s);break;
