...
export const BOMB_TITLES:Record<BombKind,string>={bomb:'爆炸牌',defuse:'拆弹',attack:'攻击',skip:'跳过',favor:'索取',shuffle:'洗牌',future:'预知三张',nope:'否决',moonCat:'月亮猫',cloudCat:'云朵猫',leafCat:'叶子猫',starCat:'星星猫',sunCat:'太阳猫'};
const HELP:Record<BombKind,string>={bomb:'没有拆弹牌就出局',defuse:'拆弹爆炸牌，并秘密放回牌堆',attack:'剩余攻击回合再加 2，交给下家',skip:'不抽牌，结束一个回合',favor:'对方选择一张手牌交给你',shuffle:'打乱抽牌堆',future:'秘密查看顶部 3 张',nope:'让一次出牌失效；再出否决可恢复效果',moonCat:'同名对子随机拿牌，三张指定牌名',cloudCat:'同名对子随机拿牌，三张指定牌名',leafCat:'同名对子随机拿牌，三张指定牌名',starCat:'同名对子随机拿牌，三张指定牌名',sunCat:'同名对子随机拿牌，三张指定牌名'};
const PLAYABLE:BombKind[]=['attack','skip','favor','shuffle','future'];
export interface BombEffect { actor:string; cards:BombCard[]; target?:string; requested?:BombKind; cancelled:boolean; passed:string[]; lastNopeBy?:string }
...
const effectTitle=(e:BombEffect)=>e.cards.length===1?BOMB_TITLES[e.cards[0].kind]:`${e.cards.length} 张同名组合`;
const choices=(cards:BombCard[]):Choice[]=>cards.map(c=>({id:c.id,title:BOMB_TITLES[c.kind],subtitle:HELP[c.kind]}));
...
function explode(s:BombsState,bomb:BombCard){const victim=s.current;s.eliminatedCards.push(...s.hands[victim],bomb);s.hands[victim]=[];s.alive=s.alive.filter(id=>id!==victim);record(s,`${name(s,victim)} 调皮离场`);s.current=nextPlayer(s,victim);s.turnsRemaining=1;s.attacked=false;s.phase={kind:'turn'};if(finished(s))record(s,`${name(s,s.alive[0])} 成为最后的留桌伙伴`);}
function beginEffect(s:BombsState,cards:BombCard[],target?:string,requested?:BombKind){const e:BombEffect={actor:s.current,cards,cancelled:false,passed:[s.current],...(target?{target}:{}),...(requested?{requested}:{})};s.hands[e.actor]=s.hands[e.actor].filter(c=>!cards.some(x=>x.id===c.id));s.discard.push(...cards);s.phase={kind:'response',effect:e};record(s,`${name(s,e.actor)} 打出${effectTitle(e)}${target?` → ${name(s,target)}`:''}`);}
function resolve(s:BombsState,e:BombEffect){
  s.phase={kind:'turn'};
  if(e.cancelled){record(s,'这次出牌未生效');return;}
...
export const bombs:GameModule<BombsState>={
  create(players,seed){
    ...
    const make=(kind:BombKind):BombCard=>({id:`bombs-card-${++s.serial}`,kind,title:BOMB_TITLES[kind]});
  ...
  view(s,playerID,spectator=false){if(spectator)playerID='';
    ...
    if(!finished(s))switch(phase.kind){
      case 'turn':if(mine){actions.push(action('draw','抽牌'));const singles=hand.filter(c=>PLAYABLE.includes(c.kind));if(singles.length)actions.push(action('play','出牌',choices(singles),1,1));for(const n of [2,3]){const combo=BOMB_KINDS.filter(k=>hand.filter(c=>c.kind===k).length>=n).map(k=>({id:k,title:`${n} 张${BOMB_TITLES[k]}`}));if(combo.length)actions.push(action(n===2?'pair':'triple',n===2?'对子':'三张',combo,1,1));}}break;
      ...
      case 'request':label='指定牌名';instruction=mine?'想要哪张牌？':`等待 ${name(s,s.current)} 指定牌名`;if(mine)actions.push(action('request','指定牌名',BOMB_KINDS.filter(k=>k!=='bomb').map(k=>({id:k,title:BOMB_TITLES[k]})),1,1),action('cancel','取消'));break;
      case 'response':{const e=phase.effect;label='响应窗口';instruction=`${name(s,e.actor)} · ${effectTitle(e)}${e.cancelled?' · 已被否决':''}`;board.response={actor:e.actor,cards:e.cards,target:e.target??null,requested:e.requested??null,cancelled:e.cancelled,passed:e.passed,lastNopeBy:e.lastNopeBy??null};sections.push({id:'responses',title:'响应',items:s.alive.map(id=>({id,title:name(s,id),detail:e.passed.includes(id)?'已确认':'待响应'}))});if(s.alive.includes(playerID)){if(!e.passed.includes(playerID))actions.push(action('pass','继续'));const nopes=hand.filter(c=>c.kind==='nope');if(nopes.length)actions.push(action('nope',e.cancelled?'恢复效果':'否决',choices(nopes),1,1));}break;}
      ...
      case 'future':label='偷瞄牌堆';instruction=mine?'顶部三张 · 仅你可见':`等待 ${name(s,s.current)}`;if(mine){board.privateFuture=phase.cards.map((c,i)=>({...c,id:`future-${i}`}));sections.push({id:'future',title:'牌堆顶',private:true,items:phase.cards.map((c,i)=>({id:`future-${i}`,title:`${i+1} · ${BOMB_TITLES[c.kind]}`}))});actions.push(action('done','继续'));}break;
      case 'bomb':label='抽到爆炸牌';instruction=`${name(s,s.current)} 抽到了爆炸牌`;if(mine)actions.push(action('defuse','拆弹'),action('explode','放弃拆弹'));break;
      case 'insert':label='秘密放回';instruction=mine?'选择爆炸牌位置':`等待 ${name(s,s.current)} 放回爆炸牌`;if(mine)actions.push(action('insert','放回爆炸牌',Array.from({length:s.deck.length+1},(_,i)=>({id:String(i),title:i===0?'最上面':i===s.deck.length?'最底下':`第 ${i+1} 张`})),1,1));break;
    }
  ...
  apply(state,playerID,command){
    ...
    switch(command.action){
      case 'draw':{if(!s.deck.length)throw Error('牌堆为空');const c=s.deck.shift()!;if(c.kind==='bomb'){record(s,`${name(s,s.current)} 抽到了爆炸牌`);if(s.hands[s.current].some(c=>c.kind==='defuse'))s.phase={kind:'bomb',card:c};else explode(s,c);}else{s.hands[s.current].push(c);record(s,`${name(s,s.current)} 抽了一张牌`);endTurn(s);}break;}
      ...
      case 'nope':{if(p.kind!=='response')throw Error('阶段无效');const i=s.hands[playerID].findIndex(c=>c.id===v[0]);s.discard.push(s.hands[playerID].splice(i,1)[0]);p.effect.cancelled=!p.effect.cancelled;p.effect.passed=[playerID];p.effect.lastNopeBy=playerID;record(s,`${name(s,playerID)} 否决 · ${p.effect.cancelled?'取消':'恢复'}效果`);break;}
      ...
      case 'insert':if(p.kind!=='insert')throw Error('阶段无效');s.deck.splice(Number(v[0]),0,p.card);record(s,`${name(s,s.current)} 已秘密放回爆炸牌`);endTurn(s);break;
