...
export const uno:GameModule<UnoState>={
 ...
 const legal=spectator?[]:actions(s,id),phase=s.finished?'本局结束':s.phase==='roundEnd'?'本轮结算':s.phase==='unoCall'?'宣告剩一张':s.phase==='unoCatch'?'漏喊响应窗口':s.phase==='penalty'?'确认罚摸':s.phase==='wild4'?'+4 接受或质疑':s.phase==='challengeResult'?'私人质疑核验':`第 ${s.roundNumber??1} 轮 · 第 ${s.round} 手 · ${s.direction===1?'顺时针':'逆时针'}`;
