...
export type UnoPhase='play'|'color'|'unoCall'|'unoCatch'|'wild4'|'challengeResult'|'roundEnd'|'penalty';
...
interface UnoWindow {actor:number;resume:'play'|'wild4'|'penalty';passed:string[]}
export interface UnoState {
 ...
 pendingPenalty?:{count:2|4;actor?:number};pendingWild4?:Wild4Pending;unoWindow?:UnoWindow;called?:Record<string,boolean>;
}
...
function dealRound(s:UnoState){
 s.deck=shuffle(cards(),seeded(s.rng++));s.hands=s.players.map(()=>s.deck.splice(-7));s.discard=[];s.direction=1;s.current=((s.dealer??s.players.length-1)+1)%s.players.length;s.color='red';s.phase='play';s.drawn=null;s.round=1;s.called={};delete s.pendingWild4;delete s.pendingPenalty;delete s.unoWindow;delete s.roundWinner;s.roundPoints=0;
 let top=s.deck.pop()!;while(top.value==='wild4'){s.deck.unshift(top);top=s.deck.pop()!;}s.discard=[top];
 if(top.color==='wild')s.phase='color';else{s.color=top.color;if(top.value==='skip')next(s);if(top.value==='draw2'){s.pendingPenalty={count:2};s.phase='penalty';}if(top.value==='reverse'){s.direction=-1;s.current=s.dealer??s.players.length-1;}}
}
function endRound(s:UnoState,index:number){if(s.hands[index].length)return;const earned=s.hands.flat().reduce((n,c)=>n+points(c),0),winner=s.players[index].id;s.scores[index]+=earned;s.roundWinner=winner;s.roundPoints=earned;(s.roundResults??=[]).push({round:s.roundNumber??1,winner,points:earned,scores:[...s.scores]});s.history.push(`${s.players[index].name} 赢得本轮 · +${earned} 分 · 累计 ${s.scores[index]} 分`);delete s.unoWindow;delete s.pendingWild4;delete s.pendingPenalty;s.drawn=null;
...
function actions(s:UnoState,id:string):Action[]{
 ...
 if(s.phase==='penalty')return[action('acceptPenalty',`接受 +${s.pendingPenalty!.count}，抽牌并跳过`)];
 ...
 for(const c of wild)result.push(action(`wild:${c.id}`,unoTitle(c),colorChoices,1,1,c.value==='wild4'?(challenges(s)?'可选择诈出 +4；若当时持有原颜色的牌，被质疑成功需自己抽 4 张':'仅无当前颜色时可出；下一家确认后抽 4 张并跳过'):'出牌时选择下一种颜色'));
...
export const uno:GameModule<UnoState>={
 ...
 view(s,id){const me=s.players.findIndex(p=>p.id===id);if(me<0)return{kind:'uno',phase:'不在本局',instruction:'仅本局玩家可查看',finished:s.finished,actions:[],sections:[],log:[],board:{}};
 const legal=actions(s,id),phase=s.finished?'本局结束':s.phase==='roundEnd'?'本轮结算':s.phase==='unoCall'?'宣告剩一张':s.phase==='unoCatch'?'漏喊响应窗口':s.phase==='penalty'?'确认罚摸':s.phase==='wild4'?'+4 接受或质疑':s.phase==='challengeResult'?'私人质疑核验':`第 ${s.roundNumber??1} 轮 · 第 ${s.round} 手 · ${s.direction===1?'顺时针':'逆时针'}`;
 const instruction=s.finished?`胜者：${s.players.find(p=>s.winners.includes(p.id))?.name}`:s.phase==='roundEnd'?`本轮胜者：${s.players.find(p=>p.id===s.roundWinner)?.name} · +${s.roundPoints} 分`:s.phase==='unoCall'?`等待 ${s.players[s.unoWindow!.actor].name} 宣告剩一张`:s.phase==='unoCatch'?'下一手开始前，确认是否指出漏喊':s.phase==='penalty'?`等待 ${s.players[s.current].name} 确认罚摸 ${s.pendingPenalty!.count} 张`:s.phase==='wild4'?`等待 ${s.players[s.pendingWild4!.target].name} 接受或质疑 +4`:s.phase==='challengeResult'?legal.length?'仅你可见：核验出牌时的手牌':'等待质疑者查看私人核验':me===s.current?s.phase==='color'?'选择起始颜色':s.drawn?'可打出刚抽到的牌，或结束回合':'轮到你了：同色、同数或功能牌':`等待 ${s.players[s.current].name}`;
 const pending=s.pendingWild4;
 return structuredClone({kind:'uno',phase,instruction,finished:s.finished,actions:legal,sections:[],log:s.history,board:{pendingPenalty:s.pendingPenalty?{count:s.pendingPenalty.count}:null,phase:s.phase,current:s.players[s.current].id,hand:s.hands[me],top:s.discard.at(-1),color:s.color,direction:s.direction,deckCount:s.deck.length,drawn:me===s.current?s.drawn:null,winners:s.winners,mode:mode(s),challengeEnabled:challenges(s),targetScore:mode(s)==='match'?500:null,roundNumber:s.roundNumber??1,roundWinner:s.roundWinner,roundPoints:s.roundPoints??0,roundResults:s.roundResults||[],
 ...
 if(command.action==='acceptPenalty'){const pending=s.pendingPenalty!;draw(s,me,pending.count);s.history.push(`${name} 接受 +${pending.count}，抽 ${pending.count} 张并跳过`);delete s.pendingPenalty;s.phase='play';next(s);if(pending.actor!==undefined)endRound(s,pending.actor);return s;}
 ...
 if(command.action==='draw'){const c=draw(s,me,1);s.history.push(`${name} 抽了 1 张`);if(c&&unoPlayable(s,c))s.drawn=c.id;else next(s);}
  ...
  if(card.value==='wild4'){next(s);if(challenges(s)){s.pendingWild4={actor:me,target:s.current,previousColor,hand:structuredClone(s.hands[me]),illegal};s.phase='wild4';}else{s.pendingPenalty={count:4,actor:me};s.phase='penalty';}}
  else if(card.value==='reverse'){s.direction=s.direction===1?-1:1;next(s,s.players.length===2?2:1);}
  else if(card.value==='skip')next(s,2);
  else if(card.value==='draw2'){next(s);s.pendingPenalty={count:2,actor:me};s.phase='penalty';}
  else next(s);
  if(s.hands[me].length===1){s.unoWindow={actor:me,resume:s.phase==='wild4'?'wild4':s.phase==='penalty'?'penalty':'play',passed:[]};s.phase='unoCall';}
  else if(!s.pendingWild4&&!s.pendingPenalty)endRound(s,me);
 }
