...
export const codenames:GameModule<SignalsState>={
 ...
 view(s,id,spectator=false){if(spectator)id='';const me=s.players.findIndex(p=>p.id===id);if(me<0&&!spectator)return{kind:'codenames',phase:say(s,'不在本局','Not in this match'),instruction:say(s,'仅本局玩家可查看','Only match participants can view'),finished:s.finished,actions:[],sections:[],log:[],board:{}};
 const captain=s.captains.includes(id),myTeam=s.teams[me],canClue=!s.finished&&s.turn===myTeam&&captain&&s.phase==='clue',canGuess=!spectator&&!s.finished&&s.turn===myTeam&&!captain&&s.phase==='guess';
