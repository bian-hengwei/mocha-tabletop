...
export const legacyMahjong:GameModule<MahjongState>={
 create(players,seed,options){assertPlayers(players,4,4);const mode=(options?.mahjongMode||'guangdong') as MahjongMode;if(!Object.hasOwn(MAHJONG_MODES,mode))throw Error('Mahjong mode is invalid');const wall=shuffle(mahjongTiles(mode),seeded(seed)),hands=players.map(()=>wall.splice(0,13));const first=wall.pop()!;hands[0].push(first);return{players:structuredClone(players),mode,hands,wall,melds:players.map(()=>[]),discards:players.map(()=>[]),current:0,phase:mode==='sichuan'||mode==='bloodflow'?'exchange':'discard',exchange:{},missing:{},pending:null,won:[],wins:[],scores:[0,0,0,0],kongPayments:[],finished:false,winners:[],history:[],drawn:first.id,selfWon:false,afterKong:false,turn:1};},
 view(s,id,spectator=false){if(spectator)id='';const me=s.players.findIndex(p=>p.id===id);if(me<0&&!spectator)return{kind:'mahjong',phase:'',instruction:'仅本局玩家可查看',finished:s.finished,actions:[],sections:[],log:[],board:{}};const legal=spectator?[]:actions(s,id);return structuredClone({kind:'mahjong',phase:s.finished?'本局结束':MAHJONG_MODES[s.mode],instruction:s.finished?(s.winners.length?`胜者：${s.players.filter(p=>s.winners.includes(p.id)).map(p=>p.name).join('、')}`:'本局和局'):s.phase==='exchange'?'选择同一花色的三张牌，交给下家':s.phase==='que'?'选择一门花色，本局优先打完':s.phase==='respond'?'等待碰杠胡响应':legal.length?'轮到你了':'等待其他玩家操作',finished:s.finished,actions:legal,sections:[],log:s.history.slice(-40),board:{assistance:spectator?null:assistance(s,me,legal),hand:[...(s.hands[me]||[])].sort((a,b)=>a.value-b.value),current:s.players[s.current].id,phase:s.phase,mode:s.mode,wallCount:s.wall.length,drawn:me===s.current?s.drawn:null,wildValue:s.mode==='laizi'?33:-1,turn:s.turn,pending:s.pending?{tile:s.pending.tile,from:s.pending.from,rob:s.pending.rob!==undefined}:null,wins:s.wins,winners:s.winners,players:s.players.map((p,i)=>({...p,count:s.hands[i].length,score:s.scores[i],won:s.won.includes(i),missing:s.phase==='que'&&i!==me?undefined:s.missing[i],discards:s.discards[i],melds:s.melds[i].map(m=>m.type==='concealed'&&i!==me&&!s.finished?{type:m.type,count:4,tiles:[],from:m.from}:m),...(s.finished?{hand:s.hands[i]}:{})}))}});},
