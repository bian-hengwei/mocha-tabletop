...
function actions(s:CenturyState):Action[]{if(s.finished)return[];const c=s.caravans[s.current],cubes=c.cubes.flatMap((n,i)=>n?[{id:String(i),title:SPICES[i],subtitle:`持有 ${n}`}]:[]);
 if(s.phase==='upgrade')return [...(s.remaining&&c.cubes.slice(0,3).some(Boolean)?[action('upgrade',`再升级 ${s.remaining} 次`,cubes.filter(x=>x.id!=='3').map(x=>({...x,title:`1 ${x.title} → 1 ${SPICES[Number(x.id)+1]}`})),1,1)]:[]),action('done','结束升级')];
...
export const century:GameModule<CenturyState>={
 ...
 view(s,id,spectator=false){if(spectator)id='';const me=s.players.findIndex(p=>p.id===id);if(me<0&&!spectator)return{kind:'century',phase:'不在本局',instruction:'仅本局玩家可查看',finished:s.finished,actions:[],sections:[],log:[],board:{}};const c=s.caravans[me]||{hand:[],played:[],cubes:zero(),orders:[]};return structuredClone({kind:'century',phase:s.finished?'游戏结束':`第 ${s.round} 轮${s.finalRound?' · 最后一轮':''}`,instruction:s.finished?`胜者：${s.players.find(p=>s.winners.includes(p.id))?.name}`:s.current===me?({action:'轮到你的商队了',pay:'选择支付的香料',trade:'继续交易，或结束本回合',upgrade:'逐次升级，或提前结束',discard:'商队最多容纳 10 枚，请归还多余香料'}[s.phase]):`等待 ${s.players[s.current].name}`,finished:s.finished,actions:me===s.current?actions(s):[],sections:[],log:s.history,board:{phase:s.phase,hand:c.hand,played:spectator?s.caravans.flatMap(c=>c.played):c.played,cubes:c.cubes,orders:c.orders,active:s.active,remaining:s.remaining,current:s.players[s.current].id,market:s.market,goals:s.goals,gold:s.gold,silver:s.silver,goalTarget:s.players.length<=3?6:5,finalRound:s.finalRound,winners:s.winners,players:s.players.map((p,i)=>({...p,cubes:s.caravans[i].cubes,handCount:s.caravans[i].hand.length,played:s.caravans[i].played,orderCount:s.caravans[i].orders.length,gold:s.caravans[i].gold,silver:s.caravans[i].silver,score:centuryScore(s.caravans[i])}))}});},
