...
export interface RoomInfo {spectators?:RoomSpectator[];allowSpectators?:boolean;expiresAt?:number;matchID?:string;code:string;kind:GameKind;mode:RoomMode;hostID:string;options?:GameOptions;players:RoomPlayer[];pending:JoinRequest[];started:boolean;revision:number}
...
export function validateMatchForRoom(value:unknown,room:RoomInfo):MatchState {
 ...
 if(room.kind==='werewolf'){
  ...
  if(optionsKey(room.kind,gameOptions,room.hostID)!==optionsKey(room.kind,expected,room.hostID))throw new Error('主持存档模式或法官身份不一致');
