...
export function normalizeGameOptions(kind:GameKind,input:unknown,hostID:string):GameOptions|undefined {
 ...
 if(Object.keys(options).some(key=>!['werewolfMode','moderatorID','language','unoMode','unoChallenge','werewolfPreset','werewolfWin'].includes(key)))throw new Error('未知游戏选项');
